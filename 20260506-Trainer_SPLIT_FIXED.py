...
class TrainConfig:
    ...
    # ── Staged destructive gate ──
    # Schedule: epochs 1..ds_warmup           → ds=0.0 (additive gate, split establishes)
    #           epochs warmup+1..warmup+ramp  → ds linearly 0.0 → ds_final
    #           epochs warmup+ramp+1..end     → ds=ds_final (full destruction)
    ds_warmup_epochs: int = 3   # epochs with ds=0 (let split establish on additive gate)
    ds_ramp_epochs: int = 3     # epochs of linear ramp from 0 to ds_final
    ds_final: float = 1.0       # target destructive_strength after ramp
...
class WatermarkTrainer:
    ...
    def _compute_destructive_strength(self, epoch: int) -> float:
        """Staged destructive_strength schedule:
           epochs 1..warmup           → 0.0   (additive only, split establishes)
           epochs warmup+1..warmup+ramp → linear 0.0 → ds_final
           epochs warmup+ramp+1..end  → ds_final
        """
        warmup = int(getattr(self.cfg, "ds_warmup_epochs", 3) or 3)
        ramp   = int(getattr(self.cfg, "ds_ramp_epochs",   3) or 3)
        final  = float(getattr(self.cfg, "ds_final",       1.0) or 1.0)
        if epoch <= warmup:
            return 0.0
        if epoch <= warmup + ramp:
            t = (epoch - warmup) / max(1, ramp)
            return float(final * t)
        return float(final)

    def _apply_destructive_strength(self, ds: float) -> None:
        """Push current ds value into both c2 and c2_ema (handles compile + DataParallel wraps)."""
        for module in (self.c2, self.c2_ema):
            inner = unwrap(module)
            try:
                inner.destructive_strength = float(ds)
            except Exception:
                pass
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            # Staged destructive_strength: ramp from 0 (epochs 1..warmup) to ds_final.
            _ds_now = self._compute_destructive_strength(epoch)
            self._apply_destructive_strength(_ds_now)
            print(f'[STAGED-DS] epoch {epoch}: destructive_strength = {_ds_now:.3f}')
                ...
                pass_floors = (
                    ...
                    getattr(self.cfg, 'best_realval_gate_specific_floor', 0.05) or 0.05)
                        and (_best_acc_raw <= _acc_raw_ceiling           # NEW: destructive gate must break clean classification
                             or self._compute_destructive_strength(epoch) < 0.5)  # — only enforced when ds >= 0.5
                )
                print(
...
def parse_args() -> TrainConfig:
    ...
    ap.add_argument("--ds_warmup_epochs", type=int,   default=3,   help="Staged destruction: epochs with ds=0 (additive only). Lets split establish first.")
    ap.add_argument("--ds_ramp_epochs",   type=int,   default=3,   help="Staged destruction: epochs of linear ramp from 0 to ds_final.")
    ap.add_argument("--ds_final",         type=float, default=1.0, help="Staged destruction: final destructive_strength after warmup+ramp.")
    ...
    cfg = TrainConfig(
        ...
        ds_warmup_epochs=int(args.ds_warmup_epochs),
        ds_ramp_epochs=int(args.ds_ramp_epochs),
        ds_final=float(args.ds_final),
    )
    return cfg
