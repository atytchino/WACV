...
class ResNet34LF_GN(nn.Module):
    ...
    def __init__(self, num_classes: int, gate_strength: float = 2.10, gn_groups: int = 32):
        ...
        # Destructive gate strength — how strongly clean classification is broken.
        # 1.0 = full sign-flip (argmax → anti-class), <1.0 = weaker, >1.0 = stronger.
        self.destructive_strength = 1.0
...
class WatermarkTrainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        # optimizers
        g_params = list(unwrap(self.mask_lat).parameters()) + list(unwrap(self.mask_64).parameters()) + list(unwrap(self.g_lat).parameters()) + list(
            unwrap(self.g_64).parameters()) + list(unwrap(self.freq_ctrl).parameters())
