...
class WatermarkTrainer:
    ...
    def step_generator(self, batch, epoch: int, it: int) -> Dict[str, float]:
        ...
        # ─────────────────────────────────────────────────────────────────────
        # DIAG 2026-05-19c: pre-step gradient diagnostic.
        ...
        if _log_grads:
            ...
            if _grad_bad_modules:
                ...
                print(f"[GRAD]   (scaler.step() should auto-skip if found_inf was set)")
        ...
        if _poison_loc is not None:
            ...
            print(f"[POISON]   --use_amp={'on' if self.use_amp else 'OFF'} "
                  f"(without AMP, GradScaler does not auto-skip NaN gradients)")
            print(f"[POISON]   Aborting to prevent silent training continuation.\n")
            raise RuntimeError(
