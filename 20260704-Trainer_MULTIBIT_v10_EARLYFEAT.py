...
class WatermarkTrainer:
    ...
    def _condition_message(self, Z: torch.Tensor, S64: torch.Tensor,
        ...
        if str(getattr(self.cfg, 'msg_inject', 'film') or 'film') == 'concat':
            _sb_lat = bits_to_signed(bits_lat)
            _sb_skip = bits_to_signed(bits_skip)
            z_pat = z_pat + self.cond_lat(Z, emb_lat, signed_bits=_sb_lat)
            w64 = w64 + self.cond_64(S64, emb_skip, signed_bits=_sb_skip)
        else:
            z_pat = z_pat + self.cond_lat(Z, emb_lat)
            w64 = w64 + self.cond_64(S64, emb_skip)
            ...
            for batch in self.train_loader:
                ...
                if (it % max(1, self.cfg.print_every) == 0):
                    ...
                    print(
                        ...
                        f"dAbs={gstats.get('delta_abs', 0.0):.4f} dMax={gstats.get('delta_max', 0.0):.4f} dMu={gstats.get('delta_mean', 0.0):+.4f} rms={gstats.get('delta_rms_pre', 0.0):.4f}->{gstats.get('delta_rms_post', 0.0):.4f} rn={gstats.get('delta_rn', 0.0):.2f} msgRMS={gstats.get('msg_rms', float('nan')):.5f} dDark={gstats.get('delta_dark_abs', 0.0):.4f} "
