...
class WatermarkTrainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        print(
            ...
            f'cond_init(film/out)={_film_std:g}/{_out_std:g} dec_conv1_stride={_dec_stride}'
        )
    ...
    def step_generator(self, batch, epoch: int, it: int) -> Dict[str, float]:
        ...
        if (it % _probe_every) == 0:
            ...
            try:
                with torch.no_grad():
                    def _probe_embed_once() -> torch.Tensor:
                        zp = self.g_lat(Z)
                        wp = self.g_64(S64)
                        zp, wp, _ = self._condition_message(Z, S64, zp, wp, stash=False)
                    ...
                    _bA = _probe_embed_once().float()
                    _bB = _probe_embed_once().float()
                    _vm = valid_mask
                    if _vm.dim() == 3:
                        _vm = _vm[:, None, :, :]
