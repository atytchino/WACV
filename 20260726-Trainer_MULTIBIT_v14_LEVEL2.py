...
class WatermarkTrainer:
    ...
    def step_generator(self, batch, epoch: int, it: int) -> Dict[str, float]:
        ...
        if (cons_lam > 0.0 and getattr(self, "cons_det", None) is not None
            ...
            composite01 = (x01[_idx2].detach() + _delta_l2).clamp(0.0, 1.0)
            _s_nat = self.cons_det(both01 * 2.0 - 1.0).flatten()
            _s_comp = self.cons_det(composite01 * 2.0 - 1.0).flatten()
            _s_orig = self.cons_det((x01.detach() * 2.0 - 1.0)).flatten()
            L_cons = (
                F.binary_cross_entropy_with_logits(_s_nat, torch.ones_like(_s_nat))
                + F.binary_cross_entropy_with_logits(_s_comp, torch.zeros_like(_s_comp))
                + 0.5 * F.binary_cross_entropy_with_logits(_s_orig, torch.zeros_like(_s_orig))
            ) / 2.5
            with torch.no_grad():
                self._last_cons_diag = (
                    float((_s_nat > 0).float().mean().item()),
                    float((_s_comp <= 0).float().mean().item()),
