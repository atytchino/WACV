...
import random
import PIL.Image as _PILImage           # or: from PIL import Image
...
from wm_dataset_configs import get_config as get_dataset_config, DATASETS
...
def unwrap(m: nn.Module) -> nn.Module:
    # Unwrap torch.compile's OptimizedModule (it has _orig_mod attribute)
    if hasattr(m, "_orig_mod"):
        m = m._orig_mod
    # Then unwrap DataParallel
    if isinstance(m, nn.DataParallel):
        m = m.module
    # Edge case: if compile was applied INSIDE DataParallel
    if hasattr(m, "_orig_mod"):
        m = m._orig_mod
    return m
...
class PadToSquareAug:
    """Training-time augmentation wrapper around PadToSquareNoUpscale.
    Augments the PIL image BEFORE pad, so valid_mask is recomputed on the
    augmented image automatically. VAL should use PadToSquareNoUpscale
    (clean). augment=False = pure pass-through (identical to original).
    """
    def __init__(self, size, pad_value, augment=False,
                 jitter=(0.15, 0.15, 0.15, 0.05), rot_deg=10.0,
                 crop_scale=(0.90, 1.0), p_flip=0.5, p_rot=0.3,
                 p_jitter=0.3, p_crop=0.3):
        self.base = PadToSquareNoUpscale(size=size, pad_value=pad_value)
        self.size = int(size)
        self.augment = bool(augment)
        self._pil_fill = self._pad_value_to_rgb(pad_value)
        self.rot_deg = float(rot_deg)
        self.crop_scale = tuple(crop_scale)
        self.p_flip = float(p_flip); self.p_rot = float(p_rot)
        self.p_jitter = float(p_jitter); self.p_crop = float(p_crop)
        b, c, s, h = jitter
        self._jitter = _T.ColorJitter(brightness=b, contrast=c,
                                      saturation=s, hue=h)

    @staticmethod
    def _pad_value_to_rgb(pad_value):
        try:
            v = float(pad_value)
        except Exception:
            v = 0.0
    ...
    def _augment_pil(self, img):
        if random.random() < self.p_flip:
            img = img.transpose(_PILImage.FLIP_LEFT_RIGHT)
        if self.rot_deg > 0 and random.random() < self.p_rot:
            angle = random.uniform(-self.rot_deg, self.rot_deg)
            img = img.rotate(angle, resample=_PILImage.BILINEAR,
                             expand=False, fillcolor=self._pil_fill)
        if random.random() < self.p_jitter:
            img = self._jitter(img)
        if random.random() < self.p_crop:
            w, h = img.size
            area = w * h
            target = random.uniform(self.crop_scale[0], self.crop_scale[1]) * area
            ratio = random.uniform(0.95, 1.05) * (w / max(1, h))
            cw = int(round((target * ratio) ** 0.5))
            ch = int(round((target / max(1e-6, ratio)) ** 0.5))
...
class RAMCachedDataset(Dataset):
    """Wraps any Dataset and caches all samples in CPU RAM on first access.

    For 600-900 images at 120×160px the entire dataset fits in ~200MB RAM.
    Eliminates all disk I/O after the first epoch — critical when training
    data lives on a spinning disk or a network drive (F:\\).

    Usage: wrap existing dataset before passing to DataLoader.
        ds = RAMCachedDataset(DiskClassFolderWithPathsAndMask(...))
    """

    def __init__(self, dataset: Dataset, verbose: bool = True):
        self.dataset = dataset
        self._cache: List = []
        self._cached = False
        if verbose:
            print(f"[RAM-CACHE] Caching {len(dataset)} samples into RAM...", flush=True)
        for i in range(len(dataset)):
            self._cache.append(dataset[i])
        self._cached = True
        if verbose:
            print(f"[RAM-CACHE] Done — {len(self._cache)} samples cached.", flush=True)
...
def _load_conditioned_generators(cfg):
    """LEVEL 2 (2026-07-26): import the FiLM content-conditioned generators from
    AE_ContentBound (they were built for exactly this swap but never wired in),
    honouring cfg.ae_py_path when the module is not on sys.path."""
    try:
        from AE_ContentBound import WMGeneratorConditioned, WMGeneratorSkip
    except ImportError:
        import sys as _sys
        _p = str(getattr(cfg, "ae_py_path", "") or "")
        if _p and _p not in _sys.path:
            _sys.path.insert(0, _p)
        from AE_ContentBound import WMGeneratorConditioned, WMGeneratorSkip
    return WMGeneratorConditioned, WMGeneratorSkip


class ConsistencyDetector(nn.Module):
    """LEVEL 2 (2026-07-26): blind content-binding consistency detector.

    ONE input — a full image in [-1, 1], no original needed — ONE logit out:
    "is the watermark on this image NATIVE to it?"

    Training classes:
        native     x_B + delta_B   -> 1  (mark generated FROM this image)
        composite  x_B + delta_A   -> 0  (a REAL mark lifted from another image)
        original   x_B             -> 0  (no mark at all)

    Anti-cheat property (the L_transfer failure mode): composites carry a
    GENUINE delta with the same marginal statistics as natives — energy,
    coherence, sign, scale are identical by construction. The ONLY thing
    separating class 1 from the composite class 0 is the JOINT (content, delta)
    pairing, so the detector cannot key on delta energy the way C2's wm_head
    does; it is forced to learn consistency. And because the generator receives
    gradient through delta in BOTH branches, it is forced to make delta a
    recognisable function of its source image. Cooperative, not adversarial:
    G and D want the same two outcomes.
    """
    ...
    def __init__(self, ch: int = 64):
        ...
        def blk(ci, co, st):
            return nn.Sequential(
                nn.Conv2d(ci, co, 3, st, 1, bias=False),
                nn.GroupNorm(_gn_groups(co), co),
                nn.SiLU(inplace=True),
            )
...
class ResNet34LF_GN(nn.Module):
    ...
    def __init__(self, num_classes: int, gate_strength: float = 2.10, gn_groups: int = 32):
        ...
        # An attacker loading the checkpoint cannot bypass destruction by zeroing
        # these — they're loaded automatically with the weights.
        # Use scalar tensors (0-dim).
        ...
        # EARLYFEAT (v8 test): detector reads early feature x1 (64ch, pre-BlurPool, pre-semantic)
        # instead of deep x4 (512ch). Hypothesis: watermark signal survives in early features.
    ...
    def forward(
        ...
        wm_logit = self.wm_head(x1).squeeze(1)  # EARLYFEAT v8: x1 (early) instead of x4 (deep)

        logits = logits_raw
        if gate:
            g = torch.tanh(wm_logit).unsqueeze(1)  # [B,1] in (-1, +1)
            if detach_gate:
                g = g.detach()
            ...
            # ── DESTRUCTIVE GATE (checkpoint-protection mode) ──
            # When g=+1  (WM)    : factor=1  → logits = logits_raw + bias        (correct argmax)
            # When g=-1  (clean) : factor=-1 → logits = -logits_raw*ds + bias    (wrong argmax)
            # The sign-flipped logits guarantee argmax lands on the least-likely class
            # for clean. ds (destructive_strength) controls how strong the inversion is;
            # higher ds = more confident wrong answer but harder to train.
            ds = getattr(self, "destructive_strength", 1.0)  # default 1.0 = full inversion
            open_factor = (g + 1.0) * 0.5  # [B,1] in [0,1], 1 for WM, 0 for clean
            close_factor = 1.0 - open_factor  # [B,1] in [0,1], 0 for WM, 1 for clean
            inv_logits = -logits_raw * ds  # sign-flipped: argmax = anti-class
...
class TrainConfig:
    ...
    # PATCH 2026-07-04 (TIER-1 experiment switch): non-zero init for the
    # ConditionedResidualBranch message pathway. Defaults 0.0/0.0 reproduce the
    # legacy double-zero init (film=0 AND out=0) that deadlocks the multi-bit
    # channel (BER stuck at exactly 0.5). Recommended: 0.01 / 0.001.
    cond_film_init_std: float = 0.0
    cond_out_init_std: float = 0.0
    # PATCH 2026-07-04 (TIER-2 queued variable): decoder conv1 stride.
    # 2 = torchvision default (legacy); 1 preserves the high-frequency
    # watermark signal (same trick as C2's base.conv1.stride=(1,1)).
    decoder_conv1_stride: int = 2
    # PATCH 2026-07-07: decoder normalization. 'bn'=BatchNorm (legacy; caught
    # cheating via batch stats — trains but VAL-BER stays 50%). 'gn'=GroupNorm
    # (per-sample, no batch dependence, no train/val stat mismatch).
    decoder_norm: str = 'bn'
    decoder_gn_groups: int = 32
    # PATCH 2026-07-07: structure-preserving tile message bridge.
    msg_bridge: str = 'none'
    tile_eps: float = 0.03
    tile_grid: str = ""  # v17: "rows,cols" to override the bit-derived grid (finer tiles). "" = auto.
    tile_logit_scale: float = 50.0
    # PATCH 2026-07-04b: msgRMS probe interval in iterations.
    # 0 = follow print_every. Probe RNG is state-isolated (see step_generator),
    # so this affects ONLY wall-time, never training dynamics.
    msg_probe_every: int = 0
    # PATCH 2026-07-06: message injection mechanism for ConditionedResidualBranch.
    # 'film' = legacy FiLM modulation (drowns after unit_rms; msgRMS≈0).
    # 'concat' = HiDDeN/MBRS-style spatial bit-tiling+concat (direct path).
    msg_inject: str = 'film'
    # PATCH 2026-07-06: dedicated message budget. The message residual and
    # the gate/content pattern were summed and shared ONE unit_rms + eps
    # budget; the gate dominated ~100x, leaving the message at ~0.3% of the
    # watermark delta (msgRMS~3e-5 vs dAbs~9e-3). alpha_msg > 0 normalizes the
    # message branch SEPARATELY and adds it as alpha_msg * msg_unit_rms AFTER
    # the gate is unit-normed, so message capacity no longer competes with the
    # gate. 0.0 = legacy summed behavior (baseline parity).
    alpha_msg: float = 0.0
    # PATCH 2026-07-06: override AE's direct image-space bridge gains after load.
    # The frozen trained AE decoder suppresses the (non-natural) message pattern
    # ~100x between latent and pixels (proven: msgFracLat=0.69 in latent →
    # msgRMS=1e-4 in pixels). The AE's built-in 'direct' bridge injects the
    # external map straight into pixels, BYPASSING the decoder; its default
    # gains (lat 0.04 / skip 0.05) are too weak. <0 = keep AE cfg default.
    ext_direct_gain_lat: float = -1.0
    ext_direct_gain_skip: float = -1.0
    ...
    ctrl_leak_max: float = 0.010  # was hardcoded 0.003; raise to let eps grow (leak-brake threshold)
    ...
    ctrl_eps_min: float = 0.02  # v16: floor for controller eps (was hardcoded 0.02); lower = weaker/less-visible delta
    ...
    # PATCH 2026-05-19: hard upper clamp on alpha (prevents the runaway alpha that drove
    # the AE decoder out of distribution and produced NaN -> black-pixel collapse).
    alpha_hard_max: float = 0.50
    ...
    wd: float = 1e-4
    ema_tau: float = 0.995
    use_augment: int = 0  # 1 = standard train-time augmentations (flip/rotate/jitter/crop); val stays clean
    ...
    # ── LEVEL 2 (2026-07-26): blind content-binding consistency ──
    content_conditioned: int = 0   # 1 = swap GLat/G64 -> WMGeneratorConditioned/WMGeneratorSkip (FiLM on content_vec)

    consistency_lam: float = 0.0   # weight of the blind native-vs-composite consistency loss (0 = off)
    consistency_start_epoch: int = 1  # epoch to enable the consistency loss
    cons_neg_types: str = "composite"  # v15: csv negatives for L_cons — composite,sign,scaled
    ...
    use_amp: bool = True  # OPT: fp16 AMP — ~35%% faster, ~40%% less VRAM on RTX 3080
    use_compile: bool = True  # OPT: torch.compile — was read via getattr, now an explicit field (DIAG 2026-05-19c)
    ram_cache: bool = True  # OPT: cache dataset in RAM (tiny datasets ≤10k imgs)
    ...
    best_realval_acc_raw_ceiling: float = 0.30  # NEW: max allowed clean (raw) accuracy for best ckpt
    # — enforces destructive gate is working on clean images
...
class WatermarkTrainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        torch.backends.cuda.matmul.allow_tf32 = True  # OPT: TF32 on Ampere (RTX 3080)
        torch.backends.cudnn.allow_tf32 = True  # OPT: TF32 in cuDNN
        torch.set_float32_matmul_precision('high')  # OPT: use TF32 for matmul
        ...
        # TRAIN: augmented transform when --use_augment is set, else identical to clean
        _use_aug = bool(getattr(cfg, "use_augment", False))
        train_tfm = PadToSquareAug(size=cfg.image_size, pad_value=cfg.pad_value, augment=_use_aug)
        _train_ds_raw = DiskClassFolderWithPathsAndMask(cfg.train_root, self.classes, train_tfm)
        _val_ds_raw = DiskClassFolderWithPathsAndMask(cfg.val_root, self.classes, self.tfm)
        # OPT: cache entire dataset in RAM — eliminates disk I/O after epoch 1.
        # IMPORTANT: with augmentation ON we must NOT cache train, or the first
        # epoch's random augmentation would be frozen and reused every epoch.
        _train_use_ram = bool(getattr(cfg, "ram_cache", True)) and not _use_aug
        self.train_ds = RAMCachedDataset(_train_ds_raw) if _train_use_ram else _train_ds_raw
        self.val_ds = RAMCachedDataset(_val_ds_raw) if bool(getattr(cfg, "ram_cache", True)) else _val_ds_raw
        ...
        self.train_loader = DataLoader(
            ...
            drop_last=True,
            persistent_workers=(cfg.num_workers > 0),  # OPT: keep workers alive
        ...
        self.val_loader = DataLoader(
            ...
            pin_memory=(self.device.type == "cuda"),
            drop_last=False,
            persistent_workers=(cfg.num_workers > 0),  # OPT
            prefetch_factor=(2 if cfg.num_workers > 0 else None),
        )
        ...
        self._apply_ext_direct_gain_override()

        # ROI + patterns
        self.mask_lat = MiniUNetMask(1024, mid=64).to(self.device)
        self.mask_64 = MiniUNetMask(512, mid=64).to(self.device)
        # LEVEL 2 (2026-07-26): optionally swap the unconditional GLat/G64 for the
        # FiLM content-conditioned generators (AE_ContentBound). delta becomes a
        # function of content_vec(Z); without this the consistency detector has
        # nothing to bind to (an unconditional delta is content-free by design).
        self._cond_gen = bool(int(getattr(cfg, "content_conditioned", 0) or 0))
        if self._cond_gen:
            _WMC, _WMS = _load_conditioned_generators(cfg)
            self.g_lat = _WMC(in_ch=1024, mid_ch=256, out_ch=1024, content_dim=64).to(self.device)
            self.g_64 = _WMS(skip_ch=512, lat_ch=1024, mid_ch=128, out_ch=512, content_dim=64).to(self.device)
            print("[LEVEL2] content-conditioned generators active: "
                  "GLat/G64 -> WMGeneratorConditioned/WMGeneratorSkip (FiLM on content_vec)")
        else:
        ...
        # LEVEL 2: blind consistency detector (native / composite / original)
        self.cons_det = None
        if float(getattr(cfg, "consistency_lam", 0.0) or 0.0) > 0.0:
            self.cons_det = ConsistencyDetector(ch=64).to(self.device)
            print("[LEVEL2] blind ConsistencyDetector instantiated "
                  "(native -> 1, composite/original -> 0)")

        # ── Multi-bit watermark components ─────────────────────
        # Resolve dataset config — defaults to ORNL grayscale if unset
        _dataset_key = getattr(cfg, 'dataset_key', 'ornl_grayscale')
        try:
            self._wm_dataset_cfg = get_dataset_config(_dataset_key)
        except KeyError:
            print(f'[WM] Unknown dataset {_dataset_key!r}, falling back to ornl_grayscale')
            self._wm_dataset_cfg = get_dataset_config('ornl_grayscale')

        _n_bits = int(getattr(cfg, 'n_bits', self._wm_dataset_cfg.decoder_n_bits))
        ...
        # PATCH 2026-07-07: tile message bridge (deterministic reader, no BN).
        self._tile_msg_signed = None
        self._tile_grid = None
        self._tile_reader = None
        if str(getattr(cfg, 'msg_bridge', 'none') or 'none') == 'tile':
            import wm_tile as _wtmod
            _tg = str(getattr(cfg, "tile_grid", "") or "").strip()
            if _tg:
                _gr, _gc = (int(x) for x in _tg.replace("x", ",").split(","))
                self._tile_grid = (_gr, _gc)
                print(f"[TILE-GRID] v17 override: grid {_gr}x{_gc} for {_n_bits} bits "
                      f"(redundant tiling, {_gr*_gc} cells)")
            else:
                self._tile_grid = _wtmod._grid_for_bits(_n_bits)
            self._tile_reader = _wtmod.TileMessageReader(
                _n_bits, grid=self._tile_grid,
                logit_scale=float(getattr(cfg, 'tile_logit_scale', 50.0) or 50.0))
            print(f'[TILE-BRIDGE] enabled: n_bits={_n_bits} grid={self._tile_grid} '
                  f'eps={float(getattr(cfg, "tile_eps", 0.03) or 0.03)}')
        # PATCH 2026-07-04: message-pathway init + decoder stride (CLI-gated)
        _film_std = float(getattr(cfg, 'cond_film_init_std', 0.0) or 0.0)
        _out_std = float(getattr(cfg, 'cond_out_init_std', 0.0) or 0.0)
        _inject = str(getattr(cfg, 'msg_inject', 'film') or 'film')
        _dec_stride = int(getattr(cfg, 'decoder_conv1_stride', 2) or 2)
        _dec_norm = str(getattr(cfg, 'decoder_norm', 'bn') or 'bn')
        _dec_gng = int(getattr(cfg, 'decoder_gn_groups', 32) or 32)
        ...
        if _use_split:
            ...
            self.cond_lat = ConditionedResidualBranch(
                ...
                film_init_std=_film_std, out_init_std=_out_std,
                inject=_inject, n_bits=_n_bits_lat,
            ).to(self.device)
            self.cond_64 = ConditionedResidualBranch(
                in_ch=512, msg_dim=_msg_dim, mid=32,
                film_init_std=_film_std, out_init_std=_out_std,
                inject=_inject, n_bits=_n_bits_skip,
            ).to(self.device)
            self.wm_decoder = SplitChannelDecoder(
                ...
                first_stride=_dec_stride, norm=_dec_norm, gn_groups=_dec_gng,
            ).to(self.device)
        else:
        ...
        print(
            ...
            f'cond_init(film/out)={_film_std:g}/{_out_std:g} dec_conv1_stride={_dec_stride} dec_norm={_dec_norm} msg_inject={_inject} msg_bridge={str(getattr(cfg,"msg_bridge","none"))}'
        )
        ...
        if cfg.use_dataparallel and self.device.type == "cuda" and torch.cuda.device_count() > 1:
            ...
            if getattr(self, "cons_det", None) is not None:
                self.cons_det = nn.DataParallel(self.cons_det)  # LEVEL 2

        # OPT: channels_last (NHWC) and torch.compile — applied AFTER full init + DP wrap
        for _name, _m in [('c2', self.c2), ('g_lat', self.g_lat), ('g_64', self.g_64)]:
            try:
                _m.to(memory_format=torch.channels_last)
            except Exception:
                pass
        if bool(getattr(cfg, 'use_compile', True)):
            # Pre-flight: torch.compile with Inductor backend requires triton.
            # Triton is not officially supported on Windows — detect and skip gracefully.
            _triton_ok = False
            try:
                import triton  # noqa: F401
                _triton_ok = True
            except ImportError:
                print("[OPT] torch.compile DISABLED: triton not installed "
                      "(expected on Windows — set --use_compile False to silence this).")
            if _triton_ok:
                # Use 'default' instead of 'reduce-overhead' — the latter enables CUDA Graphs
                # which cause "accessing tensor output that has been overwritten" errors when
                # the same compiled model is called multiple times per training step (C1 on clean,
                # C1 on WM, etc.). 'default' is slightly slower but stable in this pipeline.
                _cmode = 'default'
                _compiled = []
                _candidates = [
                    ('ae', self.ae),
                    ('g_lat', self.g_lat),
                    ('g_64', self.g_64),
                    ('mask_lat', self.mask_lat),
                    ('mask_64', self.mask_64),
                    ('freq_ctrl', self.freq_ctrl),
                    ('c2', self.c2),
                ]
                if self.c1 is not None:
                    _candidates.append(('c1', self.c1))
                for _name, _obj in _candidates:
                    try:
                        setattr(self, _name, torch.compile(_obj, mode=_cmode, fullgraph=False))
                        _compiled.append(_name)
                    except Exception as _e:
                        print(f'[OPT] torch.compile skipped for {_name}: {_e}')
                if _compiled:
        ...
        # LEVEL 2: the consistency detector's objective is COOPERATIVE with the
        # generator's (both want native->1, composite->0), so its params join
        # opt_g — one backward, one clip, one step, and AMP unscale covers it.
        if getattr(self, "cons_det", None) is not None:
            g_params = g_params + list(unwrap(self.cons_det).parameters())
        ...
        # ── AMP GradScalers (fp16 mixed precision) ──────────────────────
        # RTX 3080 Tensor Cores: fp16 gives ~35% speedup + ~40% VRAM reduction.
        # Each optimizer gets its own scaler — DataParallel-safe.
        _use_amp = (self.device.type == 'cuda') and bool(getattr(cfg, 'use_amp', True))
        self.use_amp = _use_amp
        self.scaler_g = torch.amp.GradScaler('cuda', enabled=_use_amp)
        self.scaler_c2 = torch.amp.GradScaler('cuda', enabled=_use_amp)
        if _use_amp:
            print('[OPT] AMP fp16 enabled — GradScaler active for G and C2')
        ...
        self.ctrl.eps = float(min(max(self.ctrl.eps, float(getattr(cfg, "ctrl_eps_min", 0.02) or 0.02)), float(getattr(cfg, "ctrl_eps_max", 0.12) or 0.12)))
    ...
    def load_system_ckpt(self, system_ckpt: Path) -> None:
        ...
        try:
            ...
            _ck_l2 = ckpt.get("level2_meta", {}) or {}
            raise RuntimeError(
                "[LEVEL2] generator state_dict mismatch — the checkpoint was trained with "
                f"content_conditioned={_ck_l2.get('content_conditioned', 'unknown/pre-L2')} "
                f"but this run has content_conditioned={int(getattr(self.cfg, 'content_conditioned', 0) or 0)}. "
                "Old (pre-Level-2) system checkpoints load only with --content_conditioned 0; "
                "Level-2 checkpoints load only with --content_conditioned 1. Original error: "
                + str(_e)
            )
        if "freq_ctrl" in ckpt:
            unwrap(self.freq_ctrl).load_state_dict(ckpt["freq_ctrl"], strict=True)
        # LEVEL 2: restore the blind consistency detector when the ckpt has one
        if ckpt.get("cons_det") is not None:
            if getattr(self, "cons_det", None) is None:
                self.cons_det = ConsistencyDetector(ch=64).to(self.device)
            unwrap(self.cons_det).load_state_dict(ckpt["cons_det"], strict=True)
            print("[LEVEL2] cons_det restored from checkpoint")
    ...
    def _init_infer_runtime(self) -> None:
        ...
        self.mask_lat = MiniUNetMask(1024, mid=64).to(self.device)
        self.mask_64 = MiniUNetMask(512, mid=64).to(self.device)
        # LEVEL 2 (2026-07-26): same conditional generator swap as the train init
        self._cond_gen = bool(int(getattr(cfg, "content_conditioned", 0) or 0))
        if self._cond_gen:
            _WMC, _WMS = _load_conditioned_generators(cfg)
            self.g_lat = _WMC(in_ch=1024, mid_ch=256, out_ch=1024, content_dim=64).to(self.device)
        ...
        self.freq_ctrl = FreqController(in_dim=6, hidden=32).to(self.device)
        self.lowtex = LocalVariance(k=9).to(self.device)
        self.cons_det = None  # LEVEL 2: created on checkpoint load when present
    ...
    def _compute_bandmix(self, x01: torch.Tensor, base01: torch.Tensor, valid_mask: Optional[torch.Tensor]) -> Tuple[
    ...
    def topk_binary_mask(soft: torch.Tensor, keep: float, valid: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # AMP-fix: -1e9 overflows fp16; use dtype's actual min value (safe in fp16 and fp32)
        _neg_inf = torch.finfo(flat.dtype).min
        flat_masked = flat.masked_fill(~v, _neg_inf)
    ...
    def _embed_luma_only_gray(
        ...
        # PATCH 2026-07-07 (tile message bridge): structure-preserving spatial
        # tiling injected DIRECTLY in pixels, bypassing AE/_proj_external_map
        # (channel collapse) and _blur_map. Each bit = one +/-eps luma patch.
        # Gate delta above untouched; tile added on top, inside cm.
        tile_dbg = 0.0
        if str(getattr(cfg, 'msg_bridge', 'none') or 'none') == 'tile':
            _tb = getattr(self, '_tile_msg_signed', None)
            if _tb is not None:
                import wm_tile as _wtmod
                _eps_tile = float(getattr(cfg, 'tile_eps', 0.03) or 0.03)
                _Hd, _Wd = delta.shape[-2:]
                tmap = _wtmod.tile_message_map(_tb, _Hd, _Wd, grid=getattr(self, '_tile_grid', None))
                tmap = tmap * cm
                delta = delta + _eps_tile * tmap
                tile_dbg = _eps_tile
                self._last_tile_resid = (_eps_tile * tmap).detach()

        out = (x01 + delta.repeat(1, 3, 1, 1)).clamp(0, 1)
        self._last_tile_eps_used = tile_dbg
        return out


    # ---------- message conditioning + pattern shaping (shared helpers) ----------
    # PATCH 2026-07-04: these two helpers deduplicate code that previously
    # existed as two diverging copies in step_generator and
    # synth_variants_nograd, and they are reused by the msgRMS probe.

    def _apply_ext_direct_gain_override(self):
        """PATCH 2026-07-06: push CLI ext_direct_gain_* onto the loaded AE's
        config so the image-space bridge (which bypasses the frozen decoder)
        can be strengthened without editing AE_ContentBound.py. <0 = no-op."""
        _gl = float(getattr(self.cfg, 'ext_direct_gain_lat', -1.0))
        _gs = float(getattr(self.cfg, 'ext_direct_gain_skip', -1.0))
        _ae = getattr(self, 'ae', None)
        _cfg = getattr(_ae, 'cfg', None) if _ae is not None else None
        if _cfg is None:
            return
        if _gl >= 0.0:
            _old = getattr(_cfg, 'ext_direct_gain_lat', None)
            _cfg.ext_direct_gain_lat = _gl
            print(f'[AE-OVERRIDE] ext_direct_gain_lat {_old} -> {_gl}')
        if _gs >= 0.0:
            _old = getattr(_cfg, 'ext_direct_gain_skip', None)
            _cfg.ext_direct_gain_skip = _gs
            print(f'[AE-OVERRIDE] ext_direct_gain_skip {_old} -> {_gs}')

    def _condition_message(self, Z: torch.Tensor, S64: torch.Tensor,
                           z_pat: torch.Tensor, w64: torch.Tensor,
                           stash: bool = True, override_bits=None):
        """Add message-conditioned residuals to the base patterns.

        In split mode: separate messages for latent vs skip64 paths.
        In single mode: same message goes to both (redundant encoding).

        Args:
            stash: if True (generator training step), also store the bits in
                self._last_msg_bits* for the in-step decoder loss. MUST be
                False for any no-grad synthesis (C2 step, validation, export),
                otherwise the generator-step bit state gets clobbered and the
                decoder loss would be paired with the wrong bits.

        Returns:
            (z_pat, w64, bits) where bits = {"all", "lat", "skip"} tensors.
        """
        _B = Z.shape[0]
        # PATCH 2026-07-06: override_bits lets the msgRMS probe force a SPECIFIC
        # message (needed because the probe runs under RNG isolation, which would
        # otherwise make two 'random' draws identical → probe measures 0 falsely).
        if self._wm_use_split:
            if override_bits is not None:
                bits_lat, bits_skip = override_bits
            else:
                bits_lat = random_message_bits(_B, self._wm_n_bits_lat, Z.device)
                bits_skip = random_message_bits(_B, self._wm_n_bits_skip, Z.device)
            emb_lat = self.msg_embed_lat(bits_lat)
            emb_skip = self.msg_embed_skip(bits_skip)
            bits_all = torch.cat([bits_lat, bits_skip], dim=1)
        else:
            if override_bits is not None:
                bits_all = override_bits[0] if isinstance(override_bits, tuple) else override_bits
            else:
                bits_all = random_message_bits(_B, self._wm_n_bits, Z.device)
            bits_lat = bits_all
            bits_skip = bits_all
            emb_lat = self.msg_embed(bits_all)
            emb_skip = emb_lat  # same embedding in single mode
        if stash:
            self._last_msg_bits = bits_all
            self._last_msg_bits_lat = bits_lat
            self._last_msg_bits_skip = bits_skip
        # PATCH 2026-07-07: tile bridge always stashes signed bits (independent
        # of `stash`, which governs latent decoder-loss pairing) so the embed
        # step can build the tiled pixel residual.
        if str(getattr(self.cfg, 'msg_bridge', 'none') or 'none') == 'tile':
            self._tile_msg_signed = bits_to_signed(bits_all)
        # Compute the message-conditioned residuals (parallel branches).
        # PATCH 2026-07-06: in concat mode the branch needs the signed bits
        # ({-1,+1}) tiled into channels; in film mode signed_bits is ignored.
        _concat = str(getattr(self.cfg, 'msg_inject', 'film') or 'film') == 'concat'
        if _concat:
            _sb_lat = bits_to_signed(bits_lat)
            _sb_skip = bits_to_signed(bits_skip)
            m_lat = self.cond_lat(Z, emb_lat, signed_bits=_sb_lat)
            m_64 = self.cond_64(S64, emb_skip, signed_bits=_sb_skip)
        else:
            m_lat = self.cond_lat(Z, emb_lat)
            m_64 = self.cond_64(S64, emb_skip)

        # PATCH 2026-07-06 (alpha_msg): when a dedicated message budget is set,
        # DO NOT sum the message into the gate pattern here. Stash the message
        # residuals so _shape_patterns can normalize them on their OWN unit_rms
        # and add them back with scale alpha_msg AFTER the gate is normed — this
        # stops the ~100x-stronger gate pattern from drowning the message under a
        # shared normalization. alpha_msg==0 reproduces the legacy summed path.
        _alpha_msg = float(getattr(self.cfg, 'alpha_msg', 0.0) or 0.0)
        if _alpha_msg > 0.0:
            self._msg_resid_lat = m_lat
            self._msg_resid_64 = m_64
        else:
            self._msg_resid_lat = None
            self._msg_resid_64 = None
            z_pat = z_pat + m_lat
            w64 = w64 + m_64
        return z_pat, w64, {"all": bits_all, "lat": bits_lat, "skip": bits_skip}

    def _shape_patterns(self, z_pat: torch.Tensor, w64: torch.Tensor,
                        P_lat_e: torch.Tensor, P_64_e: torch.Tensor):
        """ROI masking, anti-grid frequency shaping and unit-RMS normalization.

        Byte-for-byte the same math as the two previous inline copies.
        """
        cfg = self.cfg
        ...
        # PATCH 2026-07-06 (alpha_msg): mix in the SEPARATELY-normalized message
        # residual AFTER the gate is unit-normed. Because the message gets its
        # own unit_rms (so its scale is 1.0 regardless of how weak the raw branch
        # output is) and its own multiplier alpha_msg, it no longer competes with
        # the ~100x-stronger gate pattern under a shared normalization. Same ROI
        # masking + 3x3 smoothing as the gate path, so spatial/leak behavior
        # matches. Only active when _condition_message stashed the residuals.
        _alpha_msg = float(getattr(self.cfg, 'alpha_msg', 0.0) or 0.0)
        if _alpha_msg > 0.0 and getattr(self, '_msg_resid_lat', None) is not None:
            m_lat = F.avg_pool2d(self._msg_resid_lat * P_lat_e, 3, 1, 1)
            m_lat = unit_rms(m_lat, P_lat_e)
            z_pat = z_pat + _alpha_msg * m_lat
            if getattr(self, '_msg_resid_64', None) is not None:
                m_64 = self._msg_resid_64 * P_64_e
                m_64 = unit_rms(m_64, P_64_e)
                w64 = w64 + _alpha_msg * m_64
            # renormalize the sum so the TOTAL delta budget (what eps controls)
            # is unchanged vs legacy — the gate:message ratio is now alpha_msg,
            # but the overall watermark magnitude stays on the same eps budget.
            # PATCH 2026-07-06 diag: record the message fraction in latent z_pat
            # (BEFORE the AE) so we can tell whether the message survives shaping
            # but dies in the AE decoder, vs. never being injected. This is the
            # latent-space analogue of pixel msgRMS.
            try:
                _mnum = (( _alpha_msg * m_lat) ** 2 * P_lat_e).sum().sqrt()
                _mden = (z_pat ** 2 * P_lat_e).sum().sqrt().clamp_min(1e-9)
                self._last_msg_frac_latent = float((_mnum / _mden).item())
            except Exception:
                self._last_msg_frac_latent = float('nan')
            z_pat = unit_rms(z_pat, P_lat_e)
            w64 = unit_rms(w64, P_64_e)
            # consume the stash so a later no-grad synth can't reuse stale residuals
            self._msg_resid_lat = None
            self._msg_resid_64 = None
        return z_pat, w64
    ...
    def synth_variants_nograd(
        ...
        # patterns + message conditioning + shaping
        # PATCH 2026-07-04: deduplicated into helpers; bits are RETURNED in
        # `out` instead of mutating self._last_msg_bits*, so a no-grad
        # synthesis (C2 step / validation / export) can no longer clobber the
        # generator-step bit state, and validation BER can pair images↔bits.
        z_pat = self.g_lat(Z)
        w64 = (self.g_64(S64, Z) if getattr(self, "_cond_gen", False) else self.g_64(S64))  # LEVEL 2: skip generator needs Z for content_vec
        z_pat, w64, _msg_bits_dict = self._condition_message(Z, S64, z_pat, w64, stash=False)
        out["msg_bits"] = _msg_bits_dict["all"]
        out["msg_bits_lat"] = _msg_bits_dict["lat"]
        out["msg_bits_skip"] = _msg_bits_dict["skip"]

        # ROI + frequency shaping
        P_lat_e = self._soft_roi_mask(P_lat)
        P_64_e = self._soft_roi_mask(P_64)
        z_pat, w64 = self._shape_patterns(z_pat, w64, P_lat_e, P_64_e)
        ...
        # PATCH 2026-05-19: hard-clamp alpha to prevent unbounded growth that drives
        # the decoder out of distribution. With ext_gain_lat=4 (reduced from 16), an
        # alpha of 0.5 gives effective latent perturbation magnitude ≤ 2 (matching
        # natural latent activation magnitudes). The boost section can still scale
        # this up to ~12x for the AE-insensitive cold-start case, but bounded.
        alpha_hard_max = float(getattr(cfg, "alpha_hard_max", 0.50) or 0.50)
        if alpha_hard_max > 0.0:
            alpha_lat = alpha_lat.clamp(0.0, alpha_hard_max)
            alpha_skip = alpha_skip.clamp(0.0, alpha_hard_max)

        # PATCH 2026-07-04 (BUGFIX): this block was previously INDENTED inside
        # `if alpha_hard_max > 0.0:`, so running with --alpha_hard_max 0 silently
        # skipped the "both" variant entirely (KeyError at every out["both01"]
        # consumer). Embedding now happens unconditionally, mirroring
        # step_generator, where only the clamp is conditional.
        ...
        both01 = self._copy_outside_valid(both01, x01, valid_mask)
        # PATCH 2026-05-19: replace silent nan_to_num (which painted black pixels)
        # with safe recovery — at non-finite locations, keep the original image.
        _bad = ~torch.isfinite(both01)
        if _bad.any():
            both01 = torch.where(_bad, x01, both01)
        out["both01"] = both01
        ...
        if "lat" in variants:
            ...
            # PATCH 2026-05-19: safe NaN recovery (see comment above)
            _bad = ~torch.isfinite(lat01)
            if _bad.any():
                lat01 = torch.where(_bad, x01, lat01)
            out["lat01"] = lat01
        ...
        if "skip" in variants:
            ...
            # PATCH 2026-05-19: safe NaN recovery (see comment above)
            _bad = ~torch.isfinite(skip01)
            if _bad.any():
                skip01 = torch.where(_bad, x01, skip01)
            out["skip01"] = skip01
    ...
    def step_generator(self, batch, epoch: int, it: int) -> Dict[str, float]:
        ...
        # OPT: autocast enables fp16 Tensor Cores for all trainable forward passes
        _amp_ctx = torch.amp.autocast('cuda', enabled=self.use_amp)
        _amp_ctx.__enter__()
        ...
        # patterns + message conditioning + shaping
        # PATCH 2026-07-04: deduplicated into _condition_message / _shape_patterns
        # (shared with synth_variants_nograd and the msgRMS probe). stash=True
        # stores the bits for the in-step decoder loss below.
        z_pat = self.g_lat(Z)
        w64 = (self.g_64(S64, Z) if getattr(self, "_cond_gen", False) else self.g_64(S64))  # LEVEL 2
        z_pat, w64, _msg_bits_dict = self._condition_message(Z, S64, z_pat, w64, stash=True)
        ...
        # PATCH 2026-05-19 (revised by DIAG 2026-05-19b): NaN-guard BEFORE clamp.
        # ─────────────────────────────────────────────────────────────────────
        # CRITICAL: torch.clamp(NaN, min, max) == NaN. The original clamp
        # passed NaN straight through, making patch #3 a no-op when α was
        # already NaN (e.g., area_lat = ROI.sum() = 0 → A_lat/area_lat = 0/0).
        # We now (a) detect non-finite α with diagnostic logging, then
        # (b) replace NaN/Inf with safe finite values before the clamp.
        # ─────────────────────────────────────────────────────────────────────
        _alpha_hard_max = float(getattr(self.cfg, "alpha_hard_max", 0.50) or 0.50)
        with torch.no_grad():
            _alpha_lat_bad = bool((~torch.isfinite(alpha_lat)).any().item())
            _alpha_skip_bad = bool((~torch.isfinite(alpha_skip)).any().item())
        if _alpha_lat_bad or _alpha_skip_bad:
            if not hasattr(self, "_alpha_nan_count"):
                self._alpha_nan_count = 0
            self._alpha_nan_count += 1
            if self._alpha_nan_count <= 5 or (self._alpha_nan_count % 100 == 0):
                with torch.no_grad():
                    _al_clean = torch.nan_to_num(alpha_lat, nan=0.0, posinf=0.0, neginf=0.0)
                    _as_clean = torch.nan_to_num(alpha_skip, nan=0.0, posinf=0.0, neginf=0.0)
                    _al_min = float(_al_clean.min().item())
                    _al_max = float(_al_clean.max().item())
                    _as_min = float(_as_clean.min().item())
                    _as_max = float(_as_clean.max().item())
                    try:
                        _ar_lat = float(torch.as_tensor(area_lat).float().min().item())
                        _ar_64  = float(torch.as_tensor(area_64).float().min().item())
                    except Exception:
                        _ar_lat = _ar_64 = float('nan')
                    try:
                        _eps_v = float(eps if not torch.is_tensor(eps) else eps.float().mean().item())
                    except Exception:
                        _eps_v = float('nan')
                print(f"[ALPHA-NAN #{self._alpha_nan_count}] E{epoch} it{it}: "
                      f"lat_bad={_alpha_lat_bad} skip_bad={_alpha_skip_bad} | "
                      f"alpha_lat(clean)=[{_al_min:.3g},{_al_max:.3g}] "
                      f"alpha_skip(clean)=[{_as_min:.3g},{_as_max:.3g}] | "
                      f"area_lat_min={_ar_lat:.3g} area_64_min={_ar_64:.3g} | "
                      f"eps={_eps_v:.3g} A_lat={float(A_lat):.3g} A_64={float(A_64):.3g}")
        # Replace NaN/Inf with safe values BEFORE clamp (clamp is no-op on NaN)
        alpha_lat  = torch.nan_to_num(alpha_lat,  nan=0.0, posinf=_alpha_hard_max, neginf=0.0)
        alpha_skip = torch.nan_to_num(alpha_skip, nan=0.0, posinf=_alpha_hard_max, neginf=0.0)
        if _alpha_hard_max > 0.0:
            alpha_lat = alpha_lat.clamp(0.0, _alpha_hard_max)
            alpha_skip = alpha_skip.clamp(0.0, _alpha_hard_max)
        ...
        # PATCH 2026-05-19: replaced silent nan_to_num band-aid with detection + safe recovery.
        # ─────────────────────────────────────────────────────────────────────────────
        # OLD (silent corruption):
        #   both01 = torch.nan_to_num(both01, nan=0.0, posinf=1.0, neginf=0.0)
        #
        # The old code converted NaN/Inf to BLACK pixels (0.0), which is what created
        # the catastrophic visual collapse (dMax~0.95, SSIM~0.0005) seen in 7+ color
        # watermark training failures (TLD/AFHQ RUN03-RUN07). The NaN originated from
        # the AE decoder being driven out of distribution by latent perturbations of
        # magnitude ~16 (with original ext_gain_lat=16.0 and saturated wm_lat). Once
        # NaN reached the loss computation, gradients became corrupted.
        #
        # NEW behavior:
        #  1. Detect non-finite values BEFORE corrupting both01.
        #  2. Replace non-finite pixels with the ORIGINAL image (x01) at those locations
        #     — equivalent to "no watermark applied" rather than "destroyed to black".
        #     This keeps the image visually sensible and produces zero gradient for the
        #     affected pixels (safe — no spurious gradient signal).
        #  3. Log a rate-limited diagnostic so we know NaN is occurring (and how often).
        #  4. Track a running counter for the iteration log.
        # ─────────────────────────────────────────────────────────────────────────────
        with torch.no_grad():
            bad_mask = ~torch.isfinite(both01)  # True where NaN/Inf/etc.
            n_bad = int(bad_mask.sum().item())
        if n_bad > 0:
            # Recover: use x01 (original) at non-finite pixels — identity = "no wm" locally
            both01 = torch.where(bad_mask, x01, both01)
            # Rate-limited logging: print first occurrence per epoch, then every 50th
            if not hasattr(self, "_nan_counter_epoch"):
                self._nan_counter_epoch = 0
                self._nan_counter_total = 0
            self._nan_counter_epoch += 1
            self._nan_counter_total += 1
            if self._nan_counter_epoch <= 3 or (self._nan_counter_epoch % 50 == 0):
                try:
                    al_max = float(alpha_lat.abs().max().item()) if torch.is_tensor(alpha_lat) else float(alpha_lat)
                    as_max = float(alpha_skip.abs().max().item()) if torch.is_tensor(alpha_skip) else float(alpha_skip)
                except Exception:
                    al_max = as_max = float('nan')
                bad_frac = n_bad / max(1, both01.numel())
                print(f"[NaN-RECOVER] epoch hit #{self._nan_counter_epoch} (total #{self._nan_counter_total}): "
                      f"{n_bad} non-finite pixels ({bad_frac*100:.3f}% of batch). "
                      f"alpha_lat_max={al_max:.4f} alpha_skip_max={as_max:.4f}. "
                      f"Recovered to x01 at bad pixels.")
        # Final safety: even after recovery, ensure no NaN slipped through (paranoid check).
        # This should never trigger with the recovery above, but if it does we still keep
        # training alive (preferable to the old silent-black-pixel behavior).
        if not torch.isfinite(both01).all():
            both01 = torch.where(torch.isfinite(both01), both01, x01).clamp(0, 1)
        ...
        self.save_watermarked_batch(both01.detach(), valid_mask.detach(), paths, y, epoch,
                                    msg_bits=getattr(self, "_last_msg_bits", None))
        ...
        # ── LEVEL 2 (2026-07-26): blind native-vs-composite consistency ──
        # D sees ONLY the image (blind, no original). Negatives carry a REAL
        # delta from another image in the batch (guaranteed derangement, the
        # same cyclic-shift trick as L_transfer above), so energy statistics
        # are identical across classes and only the (content, delta) pairing
        # separates them — the anti-cheat property L_transfer lacked (it
        # judged through C2, whose wm_head keys on delta energy). Cooperative:
        # the SAME loss trains D (recognise the pairing) and G (make delta
        # content-specific enough to BE recognisable) — G receives gradient
        # through delta in both the native and the composite branch.
        L_cons = both01.new_tensor(0.0)
        cons_lam = float(getattr(self.cfg, "consistency_lam", 0.0) or 0.0)
        _cons_start = int(getattr(self.cfg, "consistency_start_epoch", 1) or 1)
        self._last_cons_diag = None
        if (cons_lam > 0.0 and getattr(self, "cons_det", None) is not None
                and epoch >= _cons_start and B > 1):
            # ── LEVEL 2 v15 (2026-07-26): EXPANDED NEGATIVES ──
            # v14 trained on ONE negative (composite x_B+δ_A: right scale, wrong
            # image) → the detector never saw −δ or α·δ, so sign_flip (47%) and
            # scaled-up passed at test time. v15 adds those as negative classes,
            # controlled by --cons_neg_types (csv of: composite,sign,scaled).
            # KEY nuance on `scaled`: α·δ is the OWNER's OWN δ amplified, so
            # penalising it trades a little owner-tolerance for scaled-attack
            # resistance — hence opt-in and only for α≥1.5 (small owner variation
            # stays native). All negatives carry a REAL/derived δ so energy still
            # cannot separate them; only the (content, δ) pairing can.
            _neg_types = str(getattr(self.cfg, "cons_neg_types", "composite") or "composite")
            _neg_set = {t.strip() for t in _neg_types.split(",") if t.strip()}
            _delta_l2 = both01 - x01.detach()                 # differentiable w.r.t. G
            _s_nat = self.cons_det(both01 * 2.0 - 1.0).flatten()
            _s_orig = self.cons_det((x01.detach() * 2.0 - 1.0)).flatten()
            _neg_logits = []
            if "composite" in _neg_set:
                _sh2 = int(torch.randint(1, B, (1,), device=x01.device).item())
                _idx2 = (torch.arange(B, device=x01.device) + _sh2) % B
                _composite01 = (x01[_idx2].detach() + _delta_l2).clamp(0.0, 1.0)
                _neg_logits.append(self.cons_det(_composite01 * 2.0 - 1.0).flatten())
            if "sign" in _neg_set:
                _signflip01 = (x01.detach() - _delta_l2).clamp(0.0, 1.0)   # x_B − δ_B
                _neg_logits.append(self.cons_det(_signflip01 * 2.0 - 1.0).flatten())
            if "scaled" in _neg_set:
                _alpha = 1.5 + 0.5 * float(torch.rand(1, device=x01.device).item())  # U[1.5,2.0]
                _scaled01 = (x01.detach() + _alpha * _delta_l2).clamp(0.0, 1.0)       # x_B + α·δ_B
                _neg_logits.append(self.cons_det(_scaled01 * 2.0 - 1.0).flatten())
            # native (label 1); every negative + original (label 0), averaged
            _L_pos = F.binary_cross_entropy_with_logits(_s_nat, torch.ones_like(_s_nat))
            _L_negs = [F.binary_cross_entropy_with_logits(l, torch.zeros_like(l)) for l in _neg_logits]
            _L_orig = 0.5 * F.binary_cross_entropy_with_logits(_s_orig, torch.zeros_like(_s_orig))
            _denom = 1.0 + float(len(_L_negs)) + 0.5
            L_cons = (_L_pos + sum(_L_negs) + _L_orig) / _denom
            with torch.no_grad():
                _comp_rej = (float((_neg_logits[0] <= 0).float().mean().item())
                             if _neg_logits else float("nan"))
                self._last_cons_diag = (
                    float((_s_nat > 0).float().mean().item()),
                    _comp_rej,
                    float((_s_orig <= 0).float().mean().item()),
                )
        ...
        L_g = (
                ...
                + (cons_lam * L_cons)  # LEVEL 2: blind native-vs-composite consistency
        )

        # ── Standalone decoder loss (recoverable bit signal) ────
        # The decoder must learn to extract the embedded message bits
        # from the watermarked image, independent of C2.
        _dec_warmup = int(getattr(self.cfg, 'decoder_warmup_epochs', 1))
        _dec_w = float(getattr(self.cfg, 'decoder_loss_weight', 1.0))
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            _tile_mode = str(getattr(self.cfg, 'msg_bridge', 'none') or 'none') == 'tile'
            if _tile_mode:
                # PATCH 2026-07-07: tile reader on luma residual (wm-clean).
                _wm_luma = self._luma01(_wm_for_decoder)
                _cl_luma = self._luma01(_clean_for_decoder)
                _resid_wm = _wm_luma - _cl_luma
                _resid_cl = torch.zeros_like(_resid_wm)
                _decoder_logits = self._tile_reader(_resid_wm)
                _decoder_logits_clean = self._tile_reader(_resid_cl)
                _msg_target = self._last_msg_bits.float()
                _L_dec_bits = F.binary_cross_entropy_with_logits(_decoder_logits, _msg_target)
                _det_wm_e = _resid_wm.abs().mean(dim=(1, 2, 3))
                _det_cl_e = _resid_cl.abs().mean(dim=(1, 2, 3))
                _det_logits = (_det_wm_e * 50.0).unsqueeze(-1)
                _det_logits_clean = (_det_cl_e * 50.0).unsqueeze(-1)
        ...
        if epoch <= _dec_warmup:
            _dec_active_w = 0.0
        else:
            _dec_active_w = _dec_w * _dec_ramp
        L_g = L_g + _dec_active_w * _L_decoder

        # ─────────────────────────────────────────────────────────────────────
        # DIAG 2026-05-19d: L_g composition check BEFORE backward.
        # If L_g itself is NaN/Inf, backward will produce NaN gradients
        # everywhere. Print each component so we can identify the bad term.
        # ─────────────────────────────────────────────────────────────────────
        with torch.no_grad():
            _L_g_finite = bool(torch.isfinite(L_g).all().item())
        if not _L_g_finite:
            if not hasattr(self, "_l_g_nan_count"):
                self._l_g_nan_count = 0
            self._l_g_nan_count += 1
            if self._l_g_nan_count <= 5 or (self._l_g_nan_count % 100 == 0):
                def _fnum(t):
                    try:
                        v = float(t.item()) if torch.is_tensor(t) else float(t)
                        return f"{v:.4f}" if (v == v and v not in (float('inf'), float('-inf'))) else f"NaN/Inf({v})"
                    except Exception:
                        return "?"
                print(f"[L_G-NAN #{self._l_g_nan_count}] E{epoch} it{it}: L_g={_fnum(L_g)} — components:")
                print(f"[L_G-NAN]   L_leak={_fnum(L_leak)} L_hf={_fnum(L_hf)} L_roi={_fnum(L_roi)} "
                      f"L_quota={_fnum(L_quota)} L_spec={_fnum(L_spec)}")
                print(f"[L_G-NAN]   L_c1_guard={_fnum(L_c1_guard)} L_grid={_fnum(L_grid)} "
                      f"L_push_cls={_fnum(L_push_cls)} L_push_det={_fnum(L_push_det)} L_push_margin={_fnum(L_push_margin)}")
                print(f"[L_G-NAN]   _last_embed_Lmin={_fnum(self._last_embed_Lmin)} L_ssim={_fnum(L_ssim)} "
                      f"L_transfer={_fnum(L_transfer)} L_diversity={_fnum(L_diversity)}"
                      + (f" | CONS nat={self._last_cons_diag[0]:.2f} compRej={self._last_cons_diag[1]:.2f} origRej={self._last_cons_diag[2]:.2f} Lcons={_fnum(L_cons)}" if getattr(self, "_last_cons_diag", None) is not None else ""))
                print(f"[L_G-NAN]   L_decoder={_fnum(_L_decoder)} dec_active_w={_dec_active_w:.4f}")

        # Track metrics for logging
        with torch.no_grad():
            self._last_bit_acc = bit_accuracy(_decoder_logits, self._last_msg_bits)
            self._last_msg_acc = message_accuracy(_decoder_logits, self._last_msg_bits)
            self._last_L_dec = float(_L_decoder.item())
            self._last_dec_active_w = float(_dec_active_w)

        _amp_ctx.__exit__(None, None, None)  # end autocast before backward
        self.opt_g.zero_grad(set_to_none=True)

        # ─────────────────────────────────────────────────────────────────────
        # DIAG 2026-05-19e: autograd anomaly detection for first 10 iterations.
        # Since L_g is finite but gradient is NaN ~98% of the time, the NaN
        # is being generated by backward through some operation (e.g., sqrt(0)
        # whose forward is 0 but backward is 1/(2*0)=inf). torch.autograd's
        # anomaly mode prints a forward-stack traceback to the offending op
        # on the first such backward. We enable it only for first ~10 iters
        # (slowdown ~10x is acceptable for diagnosis), then disable.
        # ─────────────────────────────────────────────────────────────────────
        _anomaly_window = (epoch == 1 and it < 10)
        if _anomaly_window:
            with torch.autograd.set_detect_anomaly(True, check_nan=True):
                try:
                    self.scaler_g.scale(L_g).backward()
                except RuntimeError as _anom_err:
                    print(f"\n[ANOMALY] E{epoch} it{it}: detect_anomaly caught the offending op")
                    print(f"[ANOMALY] Forward-stack traceback follows (read TOP-DOWN):")
                    print(f"[ANOMALY] {_anom_err}\n")
                    # Re-raise so we can see the full Python traceback too
                    raise
        else:
        ...
        # DIAG 2026-05-19d: Build full generator param list once (used by clip,
        # gradient diagnostic, and NaN-guard below).
        g_params_for_check = (
            list(unwrap(self.mask_lat).parameters())
            + list(unwrap(self.mask_64).parameters())
            + list(unwrap(self.g_lat).parameters())
        ...
        torch.nn.utils.clip_grad_norm_(g_params_for_check, 1.0)

        # ─────────────────────────────────────────────────────────────────────
        # DIAG 2026-05-19d: pre-step gradient diagnostic.
        # Catches the gradient state right BEFORE optimizer.step() applies it.
        # If poisoning happens this step, this log will show whether the grad
        # was non-finite (so GradScaler should have skipped), or extremely large
        # but finite (so GradScaler proceeded and Adam state corrupted), or
        # something else entirely.
        # ─────────────────────────────────────────────────────────────────────
        with torch.no_grad():
            _grad_bad_modules = []
            _grad_norms = {}
            # PATCH 2026-07-04: extended with message-branch + decoder modules
            # so the health of the multi-bit pathway is visible in [GRAD] logs
            # (previously cond_lat/cond_64/msg_embed/wm_decoder were invisible).
            _grad_diag_modules = [
                ("mask_lat", self.mask_lat), ("mask_64", self.mask_64),
                ("g_lat", self.g_lat), ("g_64", self.g_64),
                ("cond_lat", self.cond_lat), ("cond_64", self.cond_64),
                ("wm_decoder", self.wm_decoder),
            ]
            _seen_msg_ids = set()
            for _mn, _mm in (("msg_embed", getattr(self, "msg_embed", None)),
                             ("msg_embed_skip", getattr(self, "msg_embed_skip", None))):
                if _mm is not None and id(unwrap(_mm)) not in _seen_msg_ids:
                    _seen_msg_ids.add(id(unwrap(_mm)))
                    _grad_diag_modules.append((_mn, _mm))
            for _name, _mod in _grad_diag_modules:
                _max_abs = 0.0
                _any_bad = False
                for _pname, _p in unwrap(_mod).named_parameters():
                    if _p.grad is None:
                        continue
                    _g = _p.grad
                    if not torch.isfinite(_g).all():
                        _any_bad = True
                        _grad_bad_modules.append((_name, _pname,
                                                  int((~torch.isfinite(_g)).sum().item()),
                                                  int(_g.numel())))
                    _ma = float(torch.nan_to_num(_g.abs(), nan=0.0, posinf=0.0).max().item())
                    if _ma > _max_abs:
                        _max_abs = _ma
                _grad_norms[_name] = _max_abs
        # Log every 50 iterations OR always when any gradient is non-finite
        _log_grads = (it % 50 == 0) or len(_grad_bad_modules) > 0
        if _log_grads:
            _norms_s = " ".join([f"{k}={v:.3g}" for k, v in _grad_norms.items()])
            print(f"[GRAD] E{epoch} it{it} max|grad|: {_norms_s}")
            if _grad_bad_modules:
                print(f"[GRAD] NON-FINITE GRADIENTS DETECTED at E{epoch} it{it}:")
                for _name, _pname, _nbad, _ntot in _grad_bad_modules[:5]:
                    print(f"[GRAD]   {_name}.{_pname}: {_nbad}/{_ntot} non-finite")

        # ─────────────────────────────────────────────────────────────────────
        # DIAG 2026-05-19d: MANUAL NaN-grad guard.
        # ─────────────────────────────────────────────────────────────────────
        # GradScaler.step() is *supposed* to skip when found_inf is set during
        # unscale_. In the run we just saw, it skipped at it 190 & 191 but
        # PROCEEDED at it 192, even though grads were 100% NaN — possibly due
        # to a subtle interaction between unscale_, clip_grad_norm_, and the
        # scaler's per-optimizer-state tracking. Rather than debug PyTorch
        # internals, we add an explicit guard right here.
        #
        # If ANY generator param has a non-finite gradient, we zero the entire
        # gradient set (so optimizer.step becomes a no-op) AND we bypass
        # scaler.step()/scaler.update() for this iteration. Training continues
        # without weight poisoning. The downstream POISON check will not fire.
        # ─────────────────────────────────────────────────────────────────────
        with torch.no_grad():
            _any_grad_bad_for_step = False
            for _p in g_params_for_check:
                if _p.grad is not None and not torch.isfinite(_p.grad).all():
                    _any_grad_bad_for_step = True
                    break
        if _any_grad_bad_for_step:
            if not hasattr(self, "_grad_guard_count"):
                self._grad_guard_count = 0
            self._grad_guard_count += 1
            if self._grad_guard_count <= 5 or (self._grad_guard_count % 100 == 0):
                print(f"[GRAD-GUARD #{self._grad_guard_count}] E{epoch} it{it}: "
                      f"non-finite gradient detected — zeroing grads, skipping scaler.step.")
            # Zero all generator grads so any pending operation is harmless
            for _p in g_params_for_check:
                if _p.grad is not None:
                    _p.grad.zero_()
            # Still need to keep scaler's internal state consistent: calling
            # scaler.update() with no step is fine — it will simply decay scale
            # because the implicit assumption is that this iteration had a skip.
            try:
                self.scaler_g.update()
            except Exception as _e:
                # In rare cases update() may complain — log and continue
                print(f"[GRAD-GUARD] scaler.update() raised: {_e}")
        else:
            self.scaler_g.step(self.opt_g)  # AMP: safe step
            self.scaler_g.update()  # AMP: update scale

        # ─────────────────────────────────────────────────────────────────────
        # DIAG 2026-05-19b: weight-poison detector.
        # If any generator parameter becomes NaN/Inf after optimizer.step(),
        # the model is in an unrecoverable state — every subsequent forward
        # will produce NaN, NaN-recovery will silently restore wm = x01,
        # and training will "continue" with a dead generator. Abort with a
        # clear log identifying which module poisoned first.
        # ─────────────────────────────────────────────────────────────────────
        with torch.no_grad():
            _poison_loc = None
            for _name, _mod in (("mask_lat", self.mask_lat), ("mask_64", self.mask_64),
                                ("g_lat",    self.g_lat),    ("g_64",    self.g_64),
                                ("freq_ctrl", self.freq_ctrl),
                                ("cond_lat", self.cond_lat),  ("cond_64", self.cond_64),
                                ("wm_decoder", self.wm_decoder)):
                for _pname, _p in unwrap(_mod).named_parameters():
                    if not torch.isfinite(_p).all():
                        _poison_loc = (_name, _pname,
                                       int((~torch.isfinite(_p)).sum().item()),
                                       int(_p.numel()))
                        break
                if _poison_loc is not None:
                    break
        if _poison_loc is not None:
            _mname, _pname, _nbad, _ntot = _poison_loc
            _frac = 100.0 * _nbad / max(1, _ntot)
            n_recover = getattr(self, "_nan_counter_total", 0)
            n_alpha   = getattr(self, "_alpha_nan_count", 0)
            n_guard   = getattr(self, "_grad_guard_count", 0)
            print(f"\n[POISON] FATAL: generator weights non-finite after step at E{epoch} it{it}")
            print(f"[POISON]   first non-finite tensor: {_mname}.{_pname} "
                  f"({_nbad}/{_ntot} = {_frac:.2f}% bad)")
            print(f"[POISON]   cumulative counters at abort: "
                  f"NaN-recover total={n_recover}, alpha-NaN total={n_alpha}, "
                  f"grad-guard skips={n_guard}")
            print(f"[POISON]   use_amp={'on' if self.use_amp else 'off'}")
            print(f"[POISON]   This should not happen with grad-guard active — "
                  f"check whether grad-guard was bypassed.\n")
            raise RuntimeError(
                f"Weight poison detected: {_mname}.{_pname} contains "
                f"{_nbad}/{_ntot} non-finite values at E{epoch} it{it}"
            )
        ...
        # ─────────────────────────────────────────────────────────────────
        # PATCH 2026-07-04 — msgRMS probe (message-signal observability).
        # Re-embed the SAME batch twice with two fresh random messages, all
        # other factors identical (same g_lat/g_64 pattern base, same ROI,
        # same alpha), and measure the pixel-space RMS of the difference:
        #   msgRMS == 0            → the conditioned branch injects NO message
        #                            signal (the exact BER=0.5 deadlock
        #                            signature of the legacy double-zero init);
        #   msgRMS > 0 and growing → the branch is alive.
        # Runs under no_grad, only every print_every iterations, and AFTER the
        # optimizer step, so it cannot perturb training. _last_embed_dbg /
        # _last_embed_Lmin are saved/restored because _embed_luma_only_gray
        # overwrites them (Lmin is part of the NEXT iteration's loss graph).
        # Both probe embeds run in the same (fp32, post-autocast) regime, so
        # the comparison is precision-consistent by construction.
        # ─────────────────────────────────────────────────────────────────
        _probe_every = int(getattr(self.cfg, "msg_probe_every", 0) or 0)
        if _probe_every <= 0:
            _probe_every = max(1, int(getattr(self.cfg, "print_every", 50) or 50))
        if (it % _probe_every) == 0:
            _dbg_save = getattr(self, "_last_embed_dbg", None)
            _lmin_save = getattr(self, "_last_embed_Lmin", None)
            # PATCH 2026-07-04b: RNG isolation. The probe draws random message
            # bits (2x random_message_bits) from the global torch RNG; without
            # isolation the probe FREQUENCY would shift the RNG stream of the
            # actual training (r_lat draws, augmentations), making runs with
            # different print_every / msg_probe_every non-comparable. Saving
            # and restoring CPU+CUDA RNG state makes the probe a pure
            # observer: its presence and frequency affect wall-time only.
            _rng_cpu = torch.get_rng_state()
            _rng_cuda = torch.cuda.get_rng_state_all() if torch.cuda.is_available() else None
            try:
                with torch.no_grad():
                    # PATCH 2026-07-06 (BUGFIX): build TWO explicit, different
                    # messages here. Previously each _probe_embed_once relied on
                    # random_message_bits INSIDE _condition_message, but under RNG
                    # isolation both calls drew identical bits → the probe measured
                    # embed(m) - embed(m) = 0 regardless of real signal. Now we pass
                    # override_bits so the two halves genuinely differ.
                    _Bp = Z.shape[0]
                    if self._wm_use_split:
                        _mA = (random_message_bits(_Bp, self._wm_n_bits_lat, Z.device),
                               random_message_bits(_Bp, self._wm_n_bits_skip, Z.device))
                        _mB = (1.0 - _mA[0], 1.0 - _mA[1])  # bitwise-flipped => maximally different
                    else:
                        _b0 = random_message_bits(_Bp, self._wm_n_bits, Z.device)
                        _mA = _b0
                        _mB = 1.0 - _b0

                    def _probe_embed_once(_msg) -> torch.Tensor:
                        zp = self.g_lat(Z)
                        wp = (self.g_64(S64, Z) if getattr(self, "_cond_gen", False) else self.g_64(S64))  # LEVEL 2
                        zp, wp, _ = self._condition_message(Z, S64, zp, wp,
                                                            stash=False, override_bits=_msg)
                        zp, wp = self._shape_patterns(zp, wp, P_lat_e, P_64_e)
                        b01 = self._embed_luma_only_gray(
                            x01=x01, base01=base01, wm_lat=zp, wm_skip=wp,
                            alpha_lat=alpha_lat, alpha_skip=alpha_skip,
                            roi_lat=P_lat_e, roi_skip=P_64_e, valid_mask=valid_mask,
                        )
                        b01 = self._maybe_force_gray(b01, gray_mask)
                        b01 = self._copy_outside_valid(b01, x01, valid_mask)
                        return torch.where(torch.isfinite(b01), b01, x01).clamp(0, 1)

                    _bA = _probe_embed_once(_mA).float()
                    _bB = _probe_embed_once(_mB).float()
                    _vm = valid_mask
                    if _vm.dim() == 3:
                        _vm = _vm[:, None, :, :]
                    _den = (_vm.sum() * float(_bA.size(1))).clamp_min(1.0)
                    self._last_msg_rms = float(torch.sqrt(
                        (((_bA - _bB) ** 2) * _vm).sum() / _den).item())
            except Exception as _mre:
                self._last_msg_rms = float("nan")
                print(f"[msgRMS] probe failed at E{epoch} it{it}: {_mre}")
            finally:
                self._last_embed_dbg = _dbg_save if _dbg_save is not None else {}
                if _lmin_save is not None:
                    self._last_embed_Lmin = _lmin_save
        ...
        return {
            ...
            "L_cons": float(L_cons.item()),  # LEVEL 2: blind consistency
            ...
            "msg_rms": float(getattr(self, "_last_msg_rms", float("nan"))),
            "msg_frac_latent": float(getattr(self, "_last_msg_frac_latent", float("nan"))),
    ...
    def step_c2(self, batch, epoch: int) -> Dict[str, float]:
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp):  # OPT: fp16 Tensor Cores
        ...
        self.scaler_c2.scale(L_C2).backward()  # AMP: scaled backward
        if float(self.cfg.c2_grad_clip) > 0:
            self.scaler_c2.unscale_(self.opt_c2)  # AMP: unscale before clip
            torch.nn.utils.clip_grad_norm_(unwrap(self.c2).parameters(), float(self.cfg.c2_grad_clip))
        self.scaler_c2.step(self.opt_c2)  # AMP: safe step
        self.scaler_c2.update()  # AMP: update scale
    ...
    def controller_update(self, gstats: Dict[str, float], c2stats: Dict[str, float], epoch: Optional[int] = None) -> None:
        ...
        EPS_MIN = float(getattr(self.cfg, "ctrl_eps_min", 0.02) or 0.02)  # v16: was hardcoded 0.02
        EPS_MAX = float(getattr(self.cfg, "ctrl_eps_max", 0.12) or 0.12)
        LEAK_MAX = float(getattr(self.cfg, "ctrl_leak_max", 0.010) or 0.010)
        ...
        if step > 0.0:
            ...
            if (max_drop > 0.0) and (c1_clean == c1_clean) and (c1_wm == c1_wm):
                ...
                if drop > max_drop:
                    ...
                    print(
                        f"[CTRL BRAKE] E{ep_txt} reason=C1_drop clean={c1_clean:.3f} wm={c1_wm:.3f} drop={drop:.3f} max_drop={max_drop:.3f} eps={old_eps:.4f}->{new_eps:.4f}",
                        flush=True)
                    return
    ...
    def validate(self, epoch: int) -> Dict[str, float]:
        ...
        # PATCH 2026-07-04: standalone-decoder evaluation on validation.
        # This is the headline multi-bit metric (BER / message accuracy) —
        # previously never measured outside noisy per-iteration train probes.
        # NOTE: the decoder backbone uses BatchNorm (torchvision default), so
        # eval() here is REQUIRED — train-mode BN on val batches gives
        # unstable, misleading numbers. Mode is restored after the loop.
        _dec = unwrap(self.wm_decoder)
        _dec_was_training = bool(_dec.training)
        _dec.eval()
        _bit_ok = 0
        _bit_tot = 0
        _msg_ok = 0
        ...
        # PATCH 2026-07-04c: frozen C1 (naive classifier / safety circuit) on the
        # FULL validation set — both the absolute accuracy and the RAW→BOTH drop.
        # Rationale: training-time C1 probes run on train batches (memorized,
        # ~1.0) and never on val; the paper's C1 criterion needs full-val numbers.
        # Inputs are the same prod-wiped [-1,1] tensors C2 receives, matching the
        # guard's semantics. Measurement only — C1 stays frozen, no_grad.
        _c1_avail = getattr(self, "c1", None) is not None
        _c1_ok_raw = 0
        _c1_ok_both = 0
        # LEVEL 2 accumulators (blind consistency on full val)
        _cons_nat_ok = _cons_comp_rej = _cons_orig_rej = _cons_tot = 0
        ...
        for xN, valid_mask, y, paths in self.val_loader:
            ...
            # LEVEL 2: blind consistency scoring (native / composite / original).
            # Composite = REAL delta from the next image (deterministic shift-1
            # derangement, reproducible across epochs). Same unwiped [0,1]->[-1,1]
            # inputs the training loss uses.
            if getattr(self, "cons_det", None) is not None and int(y.numel()) > 1:
                with torch.no_grad():
                    _Bv = int(y.numel())
                    _idxv = (torch.arange(_Bv, device=x01.device) + 1) % _Bv
                    _delta_v = syn["both01"] - x01
                    _compN_v = ((x01[_idxv] + _delta_v).clamp(0.0, 1.0) * 2.0 - 1.0)
                    _s_nat_v = self.cons_det(syn["both01"] * 2.0 - 1.0).flatten()
                    _s_comp_v = self.cons_det(_compN_v).flatten()
                    _s_orig_v = self.cons_det(x01 * 2.0 - 1.0).flatten()
                    _cons_nat_ok += int((_s_nat_v > 0).sum().item())
                    _cons_comp_rej += int((_s_comp_v <= 0).sum().item())
                    _cons_orig_rej += int((_s_orig_v <= 0).sum().item())
                    _cons_tot += _Bv
            ...
            # PATCH 2026-07-04: decode bits from the BOTH variant; bits come
            # paired from synth_variants_nograd (out["msg_bits*"]).
            with torch.no_grad():
                _wm_in = syn["both01"].float().clamp(0, 1)
                _orig_in = x01.float().clamp(0, 1)
                _tile_mode_val = str(getattr(self.cfg, 'msg_bridge', 'none') or 'none') == 'tile'
                if _tile_mode_val:
                    _wm_luma_v = self._luma01(_wm_in)
                    _cl_luma_v = self._luma01(_orig_in)
                    _resid_v = _wm_luma_v - _cl_luma_v
                    _lg = self._tile_reader(_resid_v)
                    _pred_bits = (_lg > 0).float()
                    _true_bits = syn["msg_bits"]
                    _det_e = (_resid_v.abs().mean(dim=(1, 2, 3)) * 50.0)
                    _dlog = _det_e.unsqueeze(-1)
                    _dlog_c = torch.zeros(_resid_v.size(0), 1, device=_resid_v.device)
                elif self._wm_use_split:
                    _lg_lat, _lg_skip, _dlog = _dec.forward_full(_wm_in)
                    _unused_l, _unused_s, _dlog_c = _dec.forward_full(_orig_in)
                    _pred_bits = torch.cat([(_lg_lat > 0).float(), (_lg_skip > 0).float()], dim=1)
                    _true_bits = torch.cat([syn["msg_bits_lat"], syn["msg_bits_skip"]], dim=1)
                else:
                    _lg, _dlog = _dec.forward_full(_wm_in)
                    _unused, _dlog_c = _dec.forward_full(_orig_in)
                    _pred_bits = (_lg > 0).float()
                    _true_bits = syn["msg_bits"]
                _eq = (_pred_bits == _true_bits)
                _bit_ok += int(_eq.sum().item())
                _bit_tot += int(_eq.numel())
                _msg_ok += int(_eq.all(dim=1).sum().item())
                _msg_tot += int(_eq.size(0))
                _det_tp += int((_dlog.flatten() > 0).sum().item())
                _det_pos += int(_dlog.numel())
                _det_fp += int((_dlog_c.flatten() > 0).sum().item())
                _det_neg += int(_dlog_c.numel())

            # PATCH 2026-07-04c: C1 on RAW and BOTH (same wiped inputs as C2)
            if _c1_avail:
                with torch.no_grad():
                    _c1_pred_raw = self.c1_logits(rawN).argmax(1)
                    _c1_pred_both = self.c1_logits(bothN).argmax(1)
                _c1_ok_raw += int((_c1_pred_raw == y).sum().item())
                _c1_ok_both += int((_c1_pred_both == y).sum().item())
        ...
        # PATCH 2026-07-04: decoder metrics summary + mode restore
        val_bit_acc = _bit_ok / max(1, _bit_tot)
        val_ber = 1.0 - val_bit_acc
        val_msg_acc = _msg_ok / max(1, _msg_tot)
        dec_det_tpr = _det_tp / max(1, _det_pos)
        dec_det_fpr = _det_fp / max(1, _det_neg)
        if _dec_was_training:
            _dec.train()
        print(
            f"[VAL-BER E{epoch:02d}] BER(both)={val_ber * 100:.2f}% "
            f"bitAcc={val_bit_acc * 100:.2f}% msgAcc={val_msg_acc * 100:.2f}% | "
            f"decDet TPR={dec_det_tpr * 100:.1f}% FPR={dec_det_fpr * 100:.1f}% (thr=0) | "
            f"n_bits={self._wm_n_bits} mode={'split' if self._wm_use_split else 'single'}"
        )

        # PATCH 2026-07-04c: C1 full-val summary (paper metric: absolute + drop)
        if _c1_avail:
            val_c1_acc_raw = _c1_ok_raw / max(1, tot)
            val_c1_acc_both = _c1_ok_both / max(1, tot)
            val_c1_drop = val_c1_acc_raw - val_c1_acc_both
            print(
                f"[VAL-C1 E{epoch:02d}] RAW={val_c1_acc_raw * 100:.2f}% "
                f"BOTH={val_c1_acc_both * 100:.2f}% drop={val_c1_drop * 100:+.2f}pp "
                f"(frozen naive classifier, full val, wiped [-1,1] inputs)"
            )
        else:
            val_c1_acc_raw = float("nan")
            val_c1_acc_both = float("nan")
            val_c1_drop = float("nan")
            print(f"[VAL-C1 E{epoch:02d}] skipped (no C1 checkpoint loaded)")

        # LEVEL 2 summary: the headline is forgery_thru_cons — the fraction of
        # composites the blind detector ACCEPTS. The BEFORE system (no Level 2)
        # sits at ~99% via C2; success = this number falling toward 0 while
        # native_acc stays high and the gate metrics above stay intact.
        if _cons_tot > 0:
            val_cons_native = _cons_nat_ok / _cons_tot
            val_cons_comp_rej = _cons_comp_rej / _cons_tot
            val_cons_orig_rej = _cons_orig_rej / _cons_tot
            val_cons_forgery = 1.0 - val_cons_comp_rej
            print(
                f"[VAL-CONS E{epoch:02d}] native_acc={val_cons_native * 100:.2f}% "
                f"composite_rej={val_cons_comp_rej * 100:.2f}% "
                f"orig_rej={val_cons_orig_rej * 100:.2f}% "
                f"forgery_thru_cons={val_cons_forgery * 100:.2f}% "
                f"(blind; composite = real delta, shift-1 derangement)"
            )
        else:
            val_cons_native = float("nan")
            val_cons_comp_rej = float("nan")
            val_cons_orig_rej = float("nan")
            val_cons_forgery = float("nan")
        ...
        out = {
            ...
            "val_msg_acc": val_msg_acc,
            "val_c1_acc_raw": val_c1_acc_raw,
            "val_c1_acc_both": val_c1_acc_both,
            "val_c1_drop": val_c1_drop,
            "val_cons_native": val_cons_native,  # LEVEL 2
            "val_cons_comp_rej": val_cons_comp_rej,  # LEVEL 2
            "val_cons_orig_rej": val_cons_orig_rej,  # LEVEL 2
            "val_cons_forgery": val_cons_forgery,  # LEVEL 2: forgery through the blind consistency detector
            "dec_det_tpr": dec_det_tpr,
            "dec_det_fpr": dec_det_fpr,
    ...
    def save_watermarked_batch(self, wm01: torch.Tensor, valid_mask: torch.Tensor, paths: List[str], y: torch.Tensor, epoch: int,
                               msg_bits: Optional[torch.Tensor] = None) -> None:
        if not self.cfg.export_wm_dataset:
            return
        # Multi-bit revision: subsample to avoid I/O bottleneck (1 in N)
        _sample_every = int(getattr(self.cfg, 'wm_export_sample_every', 10) or 1)
        if not hasattr(self, '_wm_export_counter'):
            self._wm_export_counter = 0
        self._wm_export_counter += 1
        if _sample_every > 1 and (self._wm_export_counter % _sample_every) != 0:
            return
        ...
        # PATCH 2026-07-04: persist the embedded message per exported image
        # (into the epoch manifest) so offline BER evaluation on the exported
        # dataset is possible. Previously images were exported WITHOUT their
        # ground-truth bits, making the export unusable for decoder eval.
        _bits_list: List[Optional[str]] = [None] * int(wm01.size(0))
        if msg_bits is not None:
            try:
                _mb = msg_bits.detach().cpu()
                _bits_list = ["".join("1" if float(b) > 0.5 else "0" for b in row.tolist())
                              for row in _mb]
                if len(_bits_list) < int(wm01.size(0)):
                    _bits_list = _bits_list + [None] * (int(wm01.size(0)) - len(_bits_list))
            except Exception:
                _bits_list = [None] * int(wm01.size(0))

        for im, vm_i, pth, cls_idx, _bits_str in zip(wm01, valid_mask_cpu, paths, y.tolist(), _bits_list):
            ...
            self._record_export_event(
                ...
                extra={"format": fmt, "jpeg_quality": int(q),
                       "msg_bits": _bits_str,
                       "n_bits": (len(_bits_str) if _bits_str else None)},
            )
    ...
    @torch.no_grad()
    def _make_pub_collage(self, orig01: torch.Tensor, base01: torch.Tensor, both01: torch.Tensor, lat01: torch.Tensor, skip01: torch.Tensor,
                          valid_mask01: torch.Tensor) -> Tuple[Image.Image, Dict[str, object]]:
    ...
    def _selection_record_from_realval(
        ...
        passes = (worst_acc_both >= acc_floor) and (worst_det >= det_floor) and (worst_border <= border_cap) and (worst_gap >= gap_floor) and (
                    worst_gspec >= gspec_floor)
    ...
    def save_checkpoint(
        ...
        ckpt_sys = {
            ...
            "cons_det": (unwrap(self.cons_det).state_dict()
                         if getattr(self, "cons_det", None) is not None else None),  # LEVEL 2
            "level2_meta": {  # LEVEL 2 provenance
                "content_conditioned": int(getattr(self.cfg, "content_conditioned", 0) or 0),
                "consistency_lam": float(getattr(self.cfg, "consistency_lam", 0.0) or 0.0),
            },
            "last_val": val_stats,
        ...
        write_json(meta_root / f"wm_system_e{epoch:03d}.meta.json",
                   {"checkpoint": p_sys.name, "meta": ckpt_sys["meta"], "last_val": val_stats, "last_realval": real_stats})
        write_json(meta_root / f"c2_eval_e{epoch:03d}.meta.json",
                   {"checkpoint": p_c2.name, "meta": ckpt_c2["meta"], "last_val": val_stats, "last_realval": real_stats})
    ...
    def _free_cuda_memory(self, tag: str = "") -> None:
        """Aggressively release accumulated CUDA memory between epochs/validations.
        Mitigates slow OOM creep from autograd graph fragmentation, validation
        intermediate tensors, and torch.compile cache growth.
        """
        gc.collect()
        if torch.cuda.is_available():
            ...
            try:
                ...
                _resv_gb  = torch.cuda.memory_reserved()  / (1024**3)
                print(f"[MEM{(' '+tag) if tag else ''}] alloc={_alloc_gb:.2f}GB reserved={_resv_gb:.2f}GB", flush=True)
            except Exception:
                pass
    ...
    def train(self) -> None:
        ...
        train_start_wall = _time.monotonic()
        print(f"[TIMING] train() started at {_dt.now().isoformat(timespec='seconds')}", flush=True)

        # ── PATCH 2026-07-04: run provenance for predictable experiments ──
        # Dumps the FULL effective config + argv + git hash next to the
        # checkpoints, so every run is unambiguously tied to code + flags.
        try:
            import subprocess as _subp
            try:
                _git_hash = _subp.check_output(
                    ["git", "rev-parse", "HEAD"],
                    cwd=str(Path(__file__).resolve().parent),
                    stderr=_subp.DEVNULL, timeout=5,
                ).decode("utf-8", "replace").strip()
            except Exception:
                _git_hash = None
            write_json(self.ckpt_root / "meta" / "run_config.json", {
                "git_hash": _git_hash,
                "started_at_utc": utc_now_iso(),
                "argv": list(sys.argv),
                "trainer_file": str(Path(__file__).name),
                "config": json_safe(self.cfg),
            })
            print(f"[RUN-CONFIG] saved (git={_git_hash or 'n/a'})", flush=True)
        except Exception as _rce:
            print(f"[RUN-CONFIG] failed to write run_config.json: {_rce}")
        # EMA of per-batch timings (smoothed for ETA)
        _ema_t_gen = None; _ema_t_c2 = None; _ema_t_batch = None; _ema_alpha = 0.05
        for epoch in range(1, self.cfg.epochs + 1):
            epoch_start_wall = _time.monotonic()
            print(f"[TIMING E{epoch:02d}] epoch start at {_dt.now().isoformat(timespec='seconds')}", flush=True)
            # PATCH 2026-05-19: reset per-epoch NaN counter
            self._nan_counter_epoch = 0
            ...
            _prev_c2_batch = None
            _epoch_batch_count = 0
            _epoch_train_t_total = 0.0
            for batch in self.train_loader:
                it += 1
                _t_b0 = _time.monotonic()
                gstats = self.step_generator(batch, epoch, it)
                _t_g = _time.monotonic()
                c2_batch = _prev_c2_batch if _prev_c2_batch is not None else batch
                c2stats = self.step_c2(c2_batch, epoch)
                _t_c = _time.monotonic()
                _prev_c2_batch = batch
                self.controller_update(gstats, c2stats, epoch=epoch)
                _t_b1 = _time.monotonic()
                _t_gen_s = _t_g - _t_b0
                _t_c2_s  = _t_c - _t_g
                _t_batch_s = _t_b1 - _t_b0
                _epoch_batch_count += 1
                _epoch_train_t_total += _t_batch_s
                _ema_t_gen   = _t_gen_s   if _ema_t_gen is None   else _ema_alpha*_t_gen_s   + (1-_ema_alpha)*_ema_t_gen
                _ema_t_c2    = _t_c2_s    if _ema_t_c2 is None    else _ema_alpha*_t_c2_s    + (1-_ema_alpha)*_ema_t_c2
                _ema_t_batch = _t_batch_s if _ema_t_batch is None else _ema_alpha*_t_batch_s + (1-_ema_alpha)*_ema_t_batch
                ...
                if (it % max(1, self.cfg.print_every) == 0):
                    ...
                    print(
                        ...
                        f"dAbs={gstats.get('delta_abs', 0.0):.4f} dMax={gstats.get('delta_max', 0.0):.4f} dMu={gstats.get('delta_mean', 0.0):+.4f} rms={gstats.get('delta_rms_pre', 0.0):.4f}->{gstats.get('delta_rms_post', 0.0):.4f} rn={gstats.get('delta_rn', 0.0):.2f} msgRMS={gstats.get('msg_rms', float('nan')):.5f} msgFracLat={gstats.get('msg_frac_latent', float('nan')):.4f} dDark={gstats.get('delta_dark_abs', 0.0):.4f} "
            ...
            _train_dur = _t_train_end - epoch_start_wall
            _avg_per_it = (_epoch_train_t_total / max(1, _epoch_batch_count))
            print(
                f"[TIMING E{epoch:02d}] train loop done: "
                f"{_epoch_batch_count} batches in {_train_dur:.1f}s "
                f"(avg {_avg_per_it:.2f}s/it, ema {_ema_t_batch:.2f}s/it). "
                f"Now starting validation phase at {_dt.now().isoformat(timespec='seconds')}",
                flush=True,
            )
            # OOM-fix: clear prev-batch reference and free CUDA memory before validation
            _prev_c2_batch = None
            self._free_cuda_memory(tag=f"E{epoch:02d} pre-val")
            val_stats = None
            real_stats = None

            if self.cfg.val_every > 0 and (epoch % self.cfg.val_every == 0):
                print(f"[E{epoch:02d}] starting validate() at {_dt.now().isoformat(timespec='seconds')}", flush=True)
                _t_val0 = _time.monotonic()
                val_stats = self.validate(epoch)
                _t_val1 = _time.monotonic()
                print(f"[TIMING E{epoch:02d}] validate() took {(_t_val1-_t_val0):.1f}s", flush=True)
                self._free_cuda_memory(tag=f"E{epoch:02d} post-validate")

            if int(getattr(self.cfg, 'real_val_every', 0)) > 0 and (epoch % int(self.cfg.real_val_every) == 0):
                print(f"[E{epoch:02d}] starting validate_real_life() at {_dt.now().isoformat(timespec='seconds')}", flush=True)
                _t_rv0 = _time.monotonic()
                real_stats = self.validate_real_life(epoch)
                _t_rv1 = _time.monotonic()
                print(f"[TIMING E{epoch:02d}] validate_real_life() took {(_t_rv1-_t_rv0):.1f}s", flush=True)
                self._free_cuda_memory(tag=f"E{epoch:02d} post-realval")
            ...
            if real_stats is not None:
                ...
                worst_gap = (
                    (real_stats.get('mode_results', {}) or {}).get(worst_gap_name, {}) if (isinstance(real_stats, dict) and worst_gap_name) else {})
                worst_gs = (
                    (real_stats.get('mode_results', {}) or {}).get(worst_gs_name, {}) if (isinstance(real_stats, dict) and worst_gs_name) else {})
                if plain:
                    print(
                ...
                # NEW: best ckpt also requires that clean (raw) accuracy is BELOW a ceiling.
                # This guarantees the destructive gate actually breaks classification of unwatermarked
                # images at production time — protection against checkpoint manipulation attacks.
                _acc_raw_ceiling = float(getattr(self.cfg, 'best_realval_acc_raw_ceiling', 0.30) or 0.30)
                _best_acc_raw = float(real_stats.get('best_acc_raw', real_stats.get('worst_acc_raw', 1.0)) or 1.0)
                pass_floors = (
                        float(real_stats.get('worst_acc_both', float('-inf'))) >= float(
                    getattr(self.cfg, 'best_realval_acc_both_floor', 0.70) or 0.70)
                        and float(real_stats.get('worst_det_acc_raw', float('-inf'))) >= float(
                    getattr(self.cfg, 'best_realval_det_acc_floor', 0.85) or 0.85)
                        and float(real_stats.get('worst_border_ratio', float('inf'))) <= float(
                    getattr(self.cfg, 'best_realval_max_border_ratio', 0.10) or 0.10)
                        and float(real_stats.get('worst_gap_both_raw', float('-inf'))) >= float(
                    getattr(self.cfg, 'best_realval_gap_floor', 0.10) or 0.10)
                        and float(real_stats.get('worst_gate_specific_gap', float('-inf'))) >= float(
                    getattr(self.cfg, 'best_realval_gate_specific_floor', 0.05) or 0.05)
                        and _best_acc_raw <= _acc_raw_ceiling  # NEW: destructive gate must break clean classification
                )
                print(
            ...
            # OOM-fix: free CUDA memory before next epoch starts
            self._free_cuda_memory(tag=f"E{epoch:02d} post-save")

            # ── per-epoch timing summary + ETA ──
            _t_epoch_end = _time.monotonic()
            _epoch_total = _t_epoch_end - epoch_start_wall
            _val_phase   = _epoch_total - _train_dur
            _epochs_left = max(0, int(self.cfg.epochs) - epoch)
            _eta_sec = _epochs_left * _epoch_total
            _eta_h   = _eta_sec / 3600.0
            print(
                f"[TIMING E{epoch:02d}] EPOCH TOTAL = {_epoch_total:.1f}s "
                f"(train={_train_dur:.1f}s, val_phase={_val_phase:.1f}s) | "
                f"epochs_left={_epochs_left} ETA={_eta_h:.2f}h "
                f"({_eta_sec/60.0:.1f} min). "
                f"Wall now: {_dt.now().isoformat(timespec='seconds')}",
                flush=True,
            )
...
def parse_args() -> TrainConfig:
    ...
    ap.add_argument("--profile", type=str, default="auto_bandmix", choices=["auto_bandmix", "legacy"],
                    help="auto_bandmix = system chooses low/mid band per image with minimal manual tuning")
    ...
    ap.add_argument("--c1_guard_min_acc", type=float, default=0.0,
                    help="Target minimum C1 accuracy on watermarked images (used as a controller brake).")
    ...
    ap.add_argument("--c1_guard_lambda", type=float, default=0.20,
                    help="Weight of differentiable C1 guard penalty (CE degradation). 0 disables penalty.")
    ...
    ap.add_argument("--c2_init_from_c1", type=int, default=1, choices=[0, 1],
                    help="Warm-start C2 backbone+classifier from --c1_ckpt when available (wm_head/wm_affine stay fresh).")
    ...
    ap.add_argument("--clean_consistency", type=int, default=0, choices=[0, 1],
                    help="Use Variant-B clean gated consistency instead of pushing clean-gated logits to uniform.")
    ...
    ap.add_argument("--c2_keygap_margin", type=float, default=0.22,
                    help="Probability margin below 1/K for suppressing the true class in fail-clean mode.")
    ...
    ap.add_argument("--c2_early_ng_invar_mult", type=float, default=1.35,
                    help="Multiplier on non-gated invariance weight during early post-warmup epochs.")
    ap.add_argument("--c2_early_gate_specific_mult", type=float, default=1.75,
                    help="Multiplier on gate-specific loss during early post-warmup epochs.")
    ap.add_argument("--c2_early_clean_ng_keep_mult", type=float, default=1.60,
                    help="Multiplier on clean non-gated keep loss during early post-warmup epochs.")
    ...
    ap.add_argument("--c2_no_gap_ng_invar_mult", type=float, default=1.10,
                    help="Extra multiplier on non-gated invariance when detector is alive but class/gate-specific gaps lag targets.")
    ap.add_argument("--c2_no_gap_gate_specific_mult", type=float, default=1.35,
                    help="Extra multiplier on gate-specific loss when detector is alive but class/gate-specific gaps lag targets.")
    ap.add_argument("--c2_no_gap_clean_ng_keep_mult", type=float, default=1.20,
                    help="Extra multiplier on clean non-gated keep loss when detector is alive but class/gate-specific gaps lag targets.")
    ...
    # LEVEL 2 (2026-07-26): blind content-binding consistency
    ap.add_argument("--content_conditioned", type=int, default=0, help="LEVEL2: 1 = FiLM content-conditioned WM generators (required for consistency to have signal).")
    ap.add_argument("--consistency_lam", type=float, default=0.0, help="LEVEL2: weight of the blind native-vs-composite consistency loss (0 = off).")
    ap.add_argument("--consistency_start_epoch", type=int, default=1, help="LEVEL2: first epoch the consistency loss is active.")
    ap.add_argument("--cons_neg_types", type=str, default="composite", help="LEVEL2 v15: csv negatives for L_cons (composite,sign,scaled). composite=transplant, sign=-delta, scaled=alpha*delta.")
    ...
    ap.add_argument("--ctrl_init_r_skip", type=float, default=0.66,
                    help="Initial controller r_skip for train mode when no system checkpoint is loaded.")
    ap.add_argument("--ctrl_dmargin_target", type=float, default=0.06,
                    help="Target EMA delta-margin for the eps controller. Lower keeps eps from running away.")
    ...
    ap.add_argument("--ctrl_eps_min", type=float, default=0.02, help="v16: floor for controller eps (was hardcoded 0.02). Lower = weaker, less-visible watermark.")
    ap.add_argument("--ctrl_leak_max", type=float, default=0.010, help="Leak threshold for the controller brake. Was hardcoded 0.003; raise (e.g. 0.010) to let watermark strengthen.")
    ...
    ap.add_argument("--ctrl_margin_gate_specific_target", type=float, default=0.20,
                    help="Desired EMA margin-based gate-specific gap before eps is allowed to relax.")
    ...
    ap.add_argument("--use_augment", type=int, default=0,
                    help="1 = standard train-time augmentations (flip, rotation, color jitter, minimal crop); val stays clean.")
    ...
    ap.add_argument("--band_energy_norm", type=int, default=1, choices=[0, 1],
                    help="Normalize low/mid branches before auto band mixing so controller weights reflect actual energy.")
    ...
    # PATCH 2026-05-19: hard clamp on alpha to prevent unbounded growth.
    ap.add_argument("--alpha_hard_max", type=float, default=0.50,
                    help="Hard upper clamp on alpha_lat and alpha_skip (after gains and "
                         "k_factor). Set 0 to disable. Default 0.50 keeps latent perturbation "
                         "magnitude within ~2x typical activation (paired with ext_gain_lat=4 "
                         "in AE_ContentBound.py).")
    ...
    # ── Multi-bit watermarking CLI arguments ──
    ...
    # PATCH 2026-07-04: multi-bit pathway experiment switches (defaults = legacy behavior)
    ap.add_argument("--cond_film_init_std", type=float, default=0.0,
                    help="Init std for ConditionedResidualBranch FiLM weights. 0.0 = legacy zero-init (message pathway dead at start). Recommended experiment: 0.01")
    ap.add_argument("--cond_out_init_std", type=float, default=0.0,
                    help="Init std for ConditionedResidualBranch output conv. 0.0 = legacy zero-init (blocks gradient to film/conv/reduce). Recommended experiment: 0.001")
    ap.add_argument("--decoder_conv1_stride", type=int, default=2, choices=[1, 2],
                    help="Stride of the standalone decoder conv1. 2 = torchvision default (legacy); 1 preserves HF watermark signal (same trick as C2 conv1).")
    ap.add_argument("--decoder_norm", type=str, default="bn", choices=["bn", "gn"],
                    help="Decoder normalization. bn=BatchNorm (legacy; batch-stat cheating, VAL-BER stuck at 50 while train appears to learn). gn=GroupNorm (per-sample, fixes train/val mismatch; needed at batch_size=4).")
    ap.add_argument("--decoder_gn_groups", type=int, default=32,
                    help="Target GroupNorm group count for the decoder (clamped per layer to divide the channel count).")
    ap.add_argument("--msg_bridge", type=str, default="none", choices=["none", "tile"],
                    help="Message-to-pixel path. none=legacy latent/concat (AE bridge collapses channels). tile=structure-preserving spatial tiling in pixels (bypasses AE), deterministic reader. Multi-bit proof.")
    ap.add_argument("--tile_grid", type=str, default="", help="v17: override tile grid as rows,cols (e.g. 25,25 for ~10px tiles at 256px). Empty = derive from n_bits.")
    ap.add_argument("--tile_eps", type=float, default=0.03,
                    help="Per-patch luma amplitude for tile bridge (+/-eps). Try 0.02-0.05.")
    ap.add_argument("--tile_logit_scale", type=float, default=50.0, help="Logit scale for tile reader.")
    ap.add_argument("--msg_probe_every", type=int, default=0,
                    help="Run the msgRMS probe every N iterations (0 = follow print_every). Probe is RNG-isolated and post-step: affects wall-time only. With print_every=1 set e.g. 25 to avoid 2 extra AE passes per iteration.")
    ap.add_argument("--msg_inject", type=str, default="film", choices=["film", "concat"],
                    help="How the message enters the residual branch. film=legacy FiLM (empirically drowns, msgRMS~0). concat=HiDDeN/MBRS spatial bit-tiling+concat (direct gradient path, no bootstrap). Content-binding FiLM in the AE is unaffected.")
    ap.add_argument("--alpha_msg", type=float, default=0.0,
                    help="Dedicated message budget. 0=legacy (message summed into the gate pattern, shares unit_rms and drowns). >0 normalizes the message branch separately and adds alpha_msg*msg_unit_rms AFTER the gate is unit-normed, so message capacity does not compete with the gate. Try 0.3-1.0.")
    ap.add_argument("--ext_direct_gain_lat", type=float, default=-1.0,
                    help="Override AE ext_direct_gain_lat (image-space bridge that bypasses the frozen decoder). Default AE value 0.04 is too weak — the trained decoder suppresses the message ~100x. Try 0.2-0.4. <0 keeps AE default.")
    ap.add_argument("--ext_direct_gain_skip", type=float, default=-1.0,
                    help="Override AE ext_direct_gain_skip (default 0.05). <0 keeps AE default.")
    ...
    ap.add_argument("--avoid_white_thr", type=float, default=1.01,
                    help="If <=1.0, exclude near-white pixels (luma) from ROI+embedding (helps avoid highlight speckle/saturation).")
    ...
    ap.add_argument("--headroom_margin", type=float, default=0.98,
                    help="Headroom margin for saturation-safe delta clamp (0.98 keeps 2%% margin from [0,1] limits).")
    ...
    ap.add_argument("--real_val_stop_enable", type=int, default=1, choices=[0, 1],
                    help="Auto-stop training when REALVAL classification drops below (random - margin_pp).")
    ap.add_argument("--real_val_stop_margin_pp", type=float, default=5.0,
                    help="Absolute percentage-point margin below random accuracy used for REALVAL auto-stop.")
    ap.add_argument("--real_val_stop_patience", type=int, default=1, help="Number of consecutive bad REALVAL checks before stopping.")
    ap.add_argument("--real_val_stop_metric", type=str, default="both", choices=["raw", "base", "both"],
                    help="Which REALVAL classification metric to monitor for auto-stop.")
    ap.add_argument("--real_val_stop_scope", type=str, default="worst", choices=["primary", "worst"],
                    help="Use primary mode only or the worst mode across REALVAL stress suite for auto-stop.")
    ap.add_argument("--best_realval_acc_both_floor", type=float, default=0.70,
                    help="Floor on worst REALVAL acc_both for selecting the best checkpoint.")
    ap.add_argument("--best_realval_det_acc_floor", type=float, default=0.85,
                    help="Floor on worst REALVAL detector accuracy (raw-threshold) for selecting the best checkpoint.")
    ap.add_argument("--best_realval_max_border_ratio", type=float, default=0.10,
                    help="Maximum allowed worst REALVAL border_ratio for selecting the best checkpoint.")
    ap.add_argument("--best_realval_gap_floor", type=float, default=0.05,
                    help="Minimum required worst REALVAL gated gap(acc_both-acc_raw) for selecting the best checkpoint.")
    ap.add_argument("--best_realval_acc_raw_ceiling", type=float, default=0.30,
                    help="Maximum allowed clean (raw) classification accuracy for best ckpt — enforces destructive gate.")
    ap.add_argument("--best_realval_gate_specific_floor", type=float, default=0.05,
                    help="Minimum required worst REALVAL gate-specific gap = gated_gap - ungated_gap.")

    ap.add_argument("--ram_cache", type=lambda x: x.lower() != "false", default=True, help="Cache dataset in CPU RAM (recommended for <=10k images).")
    ap.add_argument("--use_amp", type=lambda x: x.lower() != "false", default=True,
                    help="Enable fp16 AMP (~35%% speedup on RTX 3080). Default: True.")
    ap.add_argument("--use_compile", type=lambda x: x.lower() != "false", default=True,
                    help="Enable torch.compile reduce-overhead — ~25%% faster. Default: True.")
    ...
    cfg = TrainConfig(
        ...
        ctrl_eps_min=float(args.ctrl_eps_min),
        ...
        ctrl_leak_max=float(args.ctrl_leak_max),
        ...
        use_augment=int(args.use_augment),
        ...
        alpha_hard_max=float(args.alpha_hard_max),  # PATCH 2026-05-19
        ...
        msg_embed_dim=int(args.msg_embed_dim),
        ...
        cond_film_init_std=float(args.cond_film_init_std),
        cond_out_init_std=float(args.cond_out_init_std),
        decoder_conv1_stride=int(args.decoder_conv1_stride),
        decoder_norm=str(args.decoder_norm),
        decoder_gn_groups=int(args.decoder_gn_groups),
        msg_bridge=str(args.msg_bridge),
        tile_eps=float(args.tile_eps),
        tile_grid=str(args.tile_grid),
        tile_logit_scale=float(args.tile_logit_scale),
        msg_probe_every=int(args.msg_probe_every),
        msg_inject=str(args.msg_inject),
        alpha_msg=float(args.alpha_msg),
        ext_direct_gain_lat=float(args.ext_direct_gain_lat),
        ext_direct_gain_skip=float(args.ext_direct_gain_skip),
        ...
        best_realval_acc_raw_ceiling=float(args.best_realval_acc_raw_ceiling),
        # DIAG 2026-05-19c: CLI BUG FIX — these were previously dropped on the floor.
        # The CLI flags were parsed into args but never propagated to TrainConfig,
        # so --use_amp False / --use_compile False were silently ignored, and cfg
        # fell back to dataclass defaults (both True). This silently re-enabled AMP
        # in every "AMP-off" debug run.
        use_amp=bool(args.use_amp),
        use_compile=bool(args.use_compile),
        # LEVEL 2 (2026-07-26): propagated explicitly — note that ssim_lam /
        # transfer_lam / diversity_lam / transfer_start_epoch CLI flags are NOT
        # propagated in this constructor (pre-existing v13 gap, same family as
        # the use_amp bug fixed 2026-05-19c): the dataclass defaults are what
        # actually run. Left untouched here to keep parity with all previous
        # runs; fix belongs to the D1 bundle with an explicit re-baseline.
        content_conditioned=int(args.content_conditioned),
        consistency_lam=float(args.consistency_lam),
        consistency_start_epoch=int(args.consistency_start_epoch),
        cons_neg_types=str(args.cons_neg_types),
    )
    return cfg
