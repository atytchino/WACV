...
═══════════════════════════════════════════════════════════════════════════════
PATCH 2026-05-19 — Color watermark catastrophic collapse fix
═══════════════════════════════════════════════════════════════════════════════
Root cause identified through systematic diagnostic (verify_ae_pipeline_v2.py):
  - 7+ failed color watermark runs (TLD/AFHQ RUN03-RUN07) collapsed because
    AE_ContentBound.ext_gain_lat=16.0 produced latent perturbations of ±16
    when wm_lat was tanh-saturated. This pushed the AE decoder far out of
    training distribution → NaN/Inf in some pixels.
  - The trainer's nan_to_num() at line 3144 silently converted NaN → 0.0
    (BLACK pixel), creating dMax≈0.95, SSIM≈0.0005 catastrophic state.
  - Confirmed: diagnostic Test 7 reproduced this state only at α=10+
    (effective gain×α=160); reasonable α with gain=4 stays bounded.

Three patches applied to fix this (all marked with "PATCH 2026-05-19"):
  1. AE_ContentBound.py: ext_gain_lat 16.0→4.0, ext_gain_skip 12.0→3.0
     (in separate file; required, but NO AE retraining needed — only runtime gain).
  2. Replace nan_to_num band-aid with NaN detection + safe recovery
     (replace bad pixels with x01 instead of 0.0; log when triggered).
  3. Hard-clamp alpha_lat and alpha_skip to max 0.50 (CLI: --alpha_hard_max).
     Prevents runaway alpha that could re-create the OOD condition.
...
class WatermarkTrainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        _train_ds_raw = DiskClassFolderWithPathsAndMask(cfg.train_root, self.classes, self.tfm)
        ...
        use_ram = bool(getattr(cfg, 'ram_cache', True))
        self.train_ds = RAMCachedDataset(_train_ds_raw) if use_ram else _train_ds_raw
        self.val_ds = RAMCachedDataset(_val_ds_raw) if use_ram else _val_ds_raw
    ...
    def step_generator(self, batch, epoch: int, it: int) -> Dict[str, float]:
        ...
        # ─────────────────────────────────────────────────────────────────────
        _anomaly_window = False  # DISABLED: was crashing on normal fp16 NaN that GradScaler handles
        if _anomaly_window:
