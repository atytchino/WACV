...
def unwrap(m: nn.Module) -> nn.Module:
    # Unwrap torch.compile's OptimizedModule first (it has _orig_mod attribute)
    if hasattr(m, "_orig_mod"):
        m = m._orig_mod
    # Then unwrap DataParallel
    if isinstance(m, nn.DataParallel):
        m = m.module
    # Edge case: if compile was applied INSIDE DataParallel (uncommon), unwrap again
    if hasattr(m, "_orig_mod"):
        m = m._orig_mod
    return m
...
class WatermarkTrainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        if bool(getattr(cfg, 'use_compile', True)):
            ...
            # Triton is not officially supported on Windows — detect missing triton and skip.
            _triton_ok = False
            try:
            ...
            if _triton_ok:
                # mode='default' — kernel fusion without CUDA graphs.
                # 'reduce-overhead' is faster but uses CUDA graphs that reuse memory →
                # output tensors get overwritten by the next forward pass, breaking
                # downstream code that holds references (e.g. .argmax() comparisons).
                _cmode = 'default'
                _compiled = []
                _candidates = [
                    ('ae',        self.ae),
...
def parse_args() -> TrainConfig:
    ...
    ap.add_argument("--use_amp",     type=lambda x: x.lower() != "false", default=True,  help="Enable fp16 AMP (~35%% speedup on RTX 3080). Default: True.")
    ap.add_argument("--use_compile", type=lambda x: x.lower() != "false", default=True,  help="Enable torch.compile reduce-overhead — ~25%% faster. Default: True.")
